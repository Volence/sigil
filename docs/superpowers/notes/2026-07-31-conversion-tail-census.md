...
| 19 | `games/demo/main.asm` | 43 | demo game manifest (include-order macros) | RESIDUAL-SKELETON | S | collapses when 14–20 are all `.emp` | **The minimal template manifest — SURVIVES (conv-hdemo).** gameConfigIncludes keeps game.asm (#15); the object-bank/data/states macros now hole out their `.emp`-native twins with ONE unconditional org each (the sonic4 deleted-twin shape). The "start here" file a new user reads first. |
