...
| 29 | `data/generated/ojz/act1/entity_data.asm` | 265 | auto-gen X-sorted ring lists + object placements + per-section type tables (BUILT, main.asm:156) | GENERATED | M | generator = `tools/ojz_entity_gen.py` | **K3 REMAINDER (data section, macros.asm's last 2 consumers):** the AS run 11D86..14B32 (with #30). objentry/objend → struct-literal `dc.w px,py,flags\|type<<TYPE_SHIFT\|sub` + `dc.w -1`; rings `dc.w X,Y` + `dc.l 0`; type tables `dc.b count,0` + `dc.l ObjDef_*`. Full native-data-section machinery. |
| 30 | `data/generated/ojz/act1/ojz_act_pool.asm` | 14 | auto-gen art-pool page BINCLUDEs + address table | GENERATED | S | generator = `tools/regenerate-level.sh` / `ojz_strip_gen.py` | **K3 REMAINDER (data section):** 3 `.zx0` page `embed()`s + `dc.l` page table (`OJZ_Act_Pool_PageTable`, consumed by act_descriptor.emp). Shares the AS run 11D86..14B32 with #29. |
