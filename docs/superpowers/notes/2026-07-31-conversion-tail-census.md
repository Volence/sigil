...
| 4 | `engine/macros.asm` | 367 | AS `function`/`macro` comptime helpers (vdpComm, vdpReg, vram_art, vram_bytes, sprSize, clearRAM, DMA macros) | RESIDUAL-SKELETON (comptime-helper carrier) | M | its `.emp` twins (engine.vdp comptime fns) largely exist; dies when its last residual-AS consumer moves | BN. **K2 UPDATE (2026-08-01): live consumers 5→2.** boot_data.asm's vdpComm/vdpReg/bytesToLcnt died with the K2 port (folded as comptime fns in boot_data.emp). The last 2 live consumers are objentry/objend (entity_data.asm, a K3 target). macros.asm deletes in K4 with its last consumer. **K3 run A UPDATE: live consumers 2→0.** entity_data.asm ported to `.emp` (objentry/objend replaced by packed typed records) — objentry/objend now have ZERO consumers (grep-proof). macros.asm is now entirely dead; its DELETION stays K4's (the file itself, engine.inc's include of it). **✅ DELETED (K4):** `engine/macros.asm` removed, engine.inc + the m1c_root.asm test harness drop the include. Byte-neutral (zero live consumers); six targets byte-identical. |
...
| 8 | ~~`engine/debug/generated/vectors.asm`~~ → `.emp`-native | 23 | golden compression self-test vectors, DEBUG-only | **DONE (conv-i8)** | S | generator = `tools/gen_compression_vectors.py` | **CONVERTED (conv-i8, 2026-08-01).** Generator now emits `engine/debug/generated/vectors.emp` (const-only `engine.compression_vectors`, the 3 CSELF_* values). The 5 golden blobs are `embed()`d directly into compression_selftest.emp (the "embed site in an existing module" shape — no new region/gate/pin/anchor; region extended [CompressionSelfTest, Sound_PostByte)). Six-target byte-identical (book-keeping-neutral: size tables + CRCs unchanged). See notes/2026-08-01-conv-i8-vectors.md. |
...
| 11 | ~~`engine/sound/generated/z80_sound_syms.asm`~~ | 59 | resident-blob exported-symbol contract (per shape) | ✅ **DELETED (Parcel K0)** — this row is STALE: seam1's `emit_sound_blob` stopped writing it at flip-stage0 (kill row 92; the seq-opcode handler VMAs now resolve in-link) and nothing includes it. The 59-line file was an untracked, gitignored (`engine/sound/generated/`) leftover from a pre-flip-stage0 build; removed from disk at K0 (nothing to commit). NOT a live sigil output — no emit-`.emp` decision remains. |

### Engine system

| # | path | lines | what | class | eff | deps / blockers | notes |
|---|---|---|---|---|---|---|---|
| 12 | `engine/system/boot_data.asm` | 131 | `BootData` (a5)+ cursor table (movem preload, VDP reg bytes, DMA fill, PSG silence, post-DMA cmds) + z80_init hole + layout-assert wall | **DONE — PORTED (K2)** | M | — | **DONE (K2, 2026-08-01):** ported to `engine/system/boot_data.emp` — TWO sections (`boot_head` = 54-byte head + the resident driver blob via `embed`; `boot_tail` = PSG silence + post-DMA cmds). The org-$3FE hole is enforced NOT by a new absolute-`org`/hole surface but by CONTIGUOUS DECLARED-ORDER packing: the map order `BootData < Z80_IdleProgram < BootData_PostBlob` places the z80_init idle in the $3d8..$3fe hole (no-sound shapes) and resumes `boot_tail` at $3fe by contiguity — the same mechanism z80_init's own port used. `BootData_End` boundary key added to all 6 frozen tables (the tail's `.emp` section carries no baked lma, so it needs the frozen pin the AS residual got for free). The conv-h2 "two-module split" IS what landed, but the chainer needed NO new surface. Assert wall → comptime `ensure`s (evenness) + the whole-ROM anchor gate (link-external waypoints). vdpComm/vdpReg/bytesToLcnt (macros.asm's last 3 non-obj consumers) fold as comptime fns here → macros.asm live consumers 5→2. **Six-target byte-clean: header-neutral assembled anchor IDENTICAL ×6; appendix-only re-freeze (new `.emp` symbols).** See `2026-08-01-k2-boot-data.md`. |
...
| # | path | lines | what | class | eff | deps / blockers | notes |
|---|---|---|---|---|---|---|---|
| 14 | `games/demo/config/constants.asm` | 14 | demo game constants + capacity contracts | PORT | S | reverse-seam (link-position) | ✅ **DONE (conv-hdemo, 2026-08-01).** `constants.asm` DELETED → `games/demo/config/constants.emp` (`games.demo.constants`), the sole authority. `demo_profile.game_constants_rel` wired → `harvest_game_constants` (the conv-f #21 mechanism, already built — no new machinery). GS_DEMO/VRAM_DEMO_OBJ/RING_*/COLLECTED_* harvested as guarded `-D` + link EquSyms (the engine ensure(extern) guards resolve); the 3 engine-VARYING `-D`s (MAX_RING_BUFFER/VRAM_RING_PLACEHOLDER/COLLECTED_WINDOW_SLOTS) stay `emp_defines`. Byte-NEUTRAL (six-ROM identity held). |
| 15 | `games/demo/config/game.asm` | 30 | demo game contract (header strings, hooks) | RESIDUAL-SKELETON | S | header string contract | **DOCUMENTED REMAINDER (conv-hdemo) — confirmed the sonic4 #22 shape.** Stays AS-residual: header-string `dc.b` data (gameHeader), gameBootHook/gameDebugTick (defined-not-invoked, game_loop lockstep), `Game_Entry`/`GAME_ENTRY_ID` (cross-seam label equalate; GAME_ENTRY_ID reads GS_DEMO via the #14 harvest), `GAME_CAMERA_JUMP_LOCK` (`-D`). No completable subset (sonic4's game.asm is the identical residual). |
...
| 17 | `games/demo/data/demo_data.asm` | 39 | objdef/mapping/art/palette | PORT (data) | S | vram_art/sprSize helpers; `objdef` construct | ✅ **DONE (conv-hdemo, 2026-08-01).** `demo_data.asm` DELETED → `games/demo/data/demo_data.emp` (`games.demo.data.demo_data`, section `demo_data`), placed natively. `objdef()` emitter + the `offsets` mapping construct (local MapPiece/MapFrame1/centered, the test_mappings shape) + struct-typed spawn list + art/palette arrays. The **demo native game-module placement path was BUILT this parcel** (demo_registry entries + code gates + frozen `demo.txt`/`demo_debug.txt` anchors) — the blocker retired. Byte-identical window `[0x10006,0x10100)` (demo_native_port). |
| 18 | `games/demo/demo_state.asm` | 36 | `GameState_Demo_Init` code | PORT (code) | S | DMA/object API externs | ✅ **DONE (conv-hdemo, 2026-08-01).** `demo_state.asm` DELETED → `games/demo/demo_state.emp` (`games.demo.demo_state`), placed natively. GameState_Demo_Init/GameState_Demo procs; cross-seam externs (Palette_Buffer, QueueDMA_Critical, InitObjectRAM, Load_ObjectList, Camera_*, Game_State, Render_Sprites…) fold at the joint link; `setVDPReg` → the VDP_MODE2_OFF shadow+dirty expansion. Byte-identical window `[0x10100,0x10172)`. |
| 19 | `games/demo/main.asm` | 43 | demo game manifest (include-order macros) | RESIDUAL-SKELETON | S | collapses when 14–20 are all `.emp` | ~~The minimal template manifest — SURVIVES (conv-hdemo).~~ **✅ DELETED (K4 inc-6B):** with every macro's content native and the orgs inert, the demo manifest is gone. The "start here" file a new game author reads is now the A1 stub `games/demo/game_root.asm` (include game.asm + debugger.asm, END — no orgs, no sound) beside `games/demo/map.toml`. Anchors identical ×6; demo full CRC UNCHANGED (its residual labels were not in the appendix). |
| 20 | `games/demo/objects/demo_box.asm` | 3 | `DemoBox_Main: jmp Draw_Sprite` | PORT (code) | S | — | ✅ **DONE (conv-hdemo, 2026-08-01).** `demo_box.asm` DELETED → `games/demo/objects/demo_box.emp` (`games.demo.demo_box`), placed natively at DemoBox_Main. `jbra Draw_Sprite` (the test_static shape). Byte-identical window `[0x10002,0x10006)`. |
...
| # | path | lines | what | class | eff | deps / blockers | notes |
|---|---|---|---|---|---|---|---|
| 28 | `data/generated/ojz/act1/bg_anim.emp` | 10 | auto-gen "no BG animation" stub | GENERATED | S | `tools/inject_editor_bg.py` | **✅ DONE (K3 run B):** `.asm`→ generated `.emp` native section `ojz_bg_anim` (`games.sonic4.ojz_bg_anim_act1`): `pub data BgAnim_Table: u16 = 0` (disabled stub) + `BgAnim_Banks = Data.empty`. Frozen key BgAnim_Table ×4 sonic4 tables; pin OJZ_BG_ANIM; map-order row; golden-gate ojz_run_b_port. Six targets FULL byte-identical. The generator's ANIMATED arm emits `.emp` too (format-faithful, not byte-proven — no animated act in the tree). |
| 29 | `data/generated/ojz/act1/entity_data.emp` | ~40 | auto-gen X-sorted ring lists + object placements + per-section type tables | GENERATED | M | `tools/ojz_entity_gen.py` | **✅ DONE (K3 run A):** `.asm`→ generated `.emp` native section `entity_data` (`games.sonic4.ojz_entity_data_act1`). objentry/objend gone: an object entry is the packed 3-word record `[x, y, flags\|(type<<8)\|subtype]` + `$FFFF` terminator (typed `[u16]`); rings `[u16]` X,Y pairs + longword-0 terminator; type tables `ObjTypeTableK` structs (count byte + pad + `count` `*u8` ObjDef pointers, packed at offset 2). Frozen key OJZ_Sec0_TypeTable (existed); pin ENTITY_DATA; golden-gate ojz_run_a_port (ObjDef seam). Six targets FULL byte-identical. **macros.asm's LAST 2 consumers gone (2→0).** |
| 30 | `data/generated/ojz/act1/ojz_act_pool.emp` | 11 | auto-gen art-pool page embeds + address table | GENERATED | S | `tools/regenerate-level.sh` | **✅ DONE (K3 run A):** `.asm`→ generated `.emp` native section `ojz_act_pool` (`games.sonic4.ojz_act_pool_act1`): 3 `.zx0` page `embed()`s + `OJZ_Act_Pool_PageTable: [*u8; 3]` (same-module `extern()` page pointers). Frozen key OJZ_Act_Pool_Page0 (NEW ×4 tables); pin OJZ_ACT_POOL; map-order row; golden-gate ojz_run_a_port. Six targets FULL byte-identical. |
| 31 | `data/generated/ojz/act1/ojz_act_pool_manifest.emp` | 9 | auto-gen pool manifest equates | GENERATED | S | `tools/ojz_strip_gen.py` | **✅ DONE (K3):** `.asm`→ generated `.emp` const module (`games.sonic4.ojz_act_pool_manifest_act1`, `pub const`s); act_descriptor.emp `use`s OJZ_ACT_POOL_PAGES/TILES at comptime; the pool-tile ceiling wall folded into a comptime `ensure`. Zero ROM bytes → no section/pin. Six targets FULL byte-identical. |
| 32 | `data/generated/ojz/act1/sec_block_blobs.emp` | 17 | auto-gen per-section block-blob embeds (content-dedup) | GENERATED | S | `tools/ojz_block_gen.py` | **✅ DONE (K3 run B):** `.asm`→ generated `.emp` native section `sec_block_blobs` (`games.sonic4.ojz_sec_block_blobs_act1`): 8 `embed()`s + `OJZ_Sec4_Blocks = extern("OJZ_Sec2_Blocks")` dedup alias (zero bytes). Frozen key OJZ_Sec0_Blocks (existed); pin SEC_BLOCK_BLOBS; golden-gate ojz_run_b_port. Six targets FULL byte-identical. |

**K3 run-B palette/BG BINCLUDE dissolution (census-missed, act_descriptor.asm interior):** the four direct BINCLUDEs that used to sit in `act_descriptor.asm` after the descriptor — `OJZ_Palette` (ojz_palette.bin), `BGND_Palette` (art/palettes/SonicAndTails.bin), `OJZ_Act1_BG_Layout` (zone_bg.bin), `OJZ_Act1_BG_Tiles` (bg_tiles.bin) — **✅ DONE (K3 run B):** dissolved into a hand-authored native `.emp` section `ojz_act_assets` (`games.sonic4.ojz_act_assets_act1`, `games/sonic4/data/levels/ojz/act1/act_assets.emp`, 4 `embed()`s). Frozen key OJZ_Palette ×4 sonic4 tables; pin OJZ_ACT_ASSETS; map-order row; golden-gate ojz_run_b_port.
| 33 | `data/generated/ojz/act1/sec_block_dicts.emp` | 11 | auto-gen block-dictionary length equates | GENERATED | S | `tools/ojz_block_gen.py` | **✅ DONE (K3):** `.asm`→ generated `.emp` const module (`games.sonic4.ojz_block_dicts_act1`, `pub const`s); act_descriptor.emp `use`s the 9 OJZ_SEC*_BLOCK_DICT_LEN at comptime. Zero ROM bytes → no section/pin. Six targets FULL byte-identical. |

### games/sonic4/data/levels (committed, portable, BUILT)

| # | path | lines | what | class | eff | deps / blockers | notes |
|---|---|---|---|---|---|---|---|
| 34 | `data/levels/ojz/act1/act_descriptor.asm` | 268 | level descriptor + section grid tables (uses `Act`/`Sec` struct; `include`s the generated pool/dict/blob files) | PORT / DATA-DSL | M | **structs flip (#3)** for the struct twins; `[Act;N]`/`[Sec;N]` struct-array DSL; the generated includes (#30–33) | BN. **✅ DONE (conv-h #34):** the descriptor+sections were already the canonical typed `Act`/`[Sec;9]` literal in `act_descriptor.emp` (gate on every sonic4 build); the dead `ifndef` AS twin + duplicated grid/axis asserts DELETED. Generated includes/BINCLUDEs + org resume stay AS-side. Six targets byte-identical (kill row 93). **✅ FULLY DELETED (K3 run A):** with #30 (the last include) native, the wrapper had nothing left — `act_descriptor.asm` DELETED, its main.asm:157 include removed, its org-resume pair gone with run B. The OJZ level tree is now 100% `.emp` (zero `.asm`). |
...
| # | path | lines | what | class | eff | deps / blockers | notes |
|---|---|---|---|---|---|---|---|
| 50 | `games/sonic4/main.asm` | 343 | game manifest: include-order macros, ~15 INERT resume `org`s, the collision-data BINCLUDE island (HeightMaps/AngleTable/SolidityTable/Map_Sonic/DPLC_Sonic/Art_Sonic), the sound-data BINCLUDE island (DAC/MT/SFX) | RESIDUAL-SKELETON | L (capstone) | the residual-split companion (ledger 1852) + all data islands moved to `.emp` | **CAPSTONE-SIZED.** The orgs are inert; the BINCLUDE islands are data. Dies via the ledgered residual-split. `engine/engine.inc` (`.inc`, not in the 50) is the engine half of the same capstone. **✅ DELETED (K4 inc-6B):** every data island native by K3/K4 inc-5, every org inert, the last hand-written bytes native by inc-6A — so `games/sonic4/main.asm` is gone. The AS root is the A1 stub `games/sonic4/game_root.asm` (include game.asm + debugger.asm + the SIGIL_EMP_MT-gated mt_syms re-home). The 7 game*Includes macros die with the file. Anchors identical ×6; appendix-only re-freeze (chain #18, MovingTrucks_Bank_Start left the deb2 symbol dump). |
| 50b | `engine/engine.inc` | 459 | the engine ROM-layout root: `cpu/padding/supmode`, the 106-org ladder, the game*Includes invocations, `include debugger.asm`, the gated ObjCodeBase/NullInterrupt/EndOfRom byte emitters | RESIDUAL-SKELETON (engine half of #50) | L (capstone) | every included module + byte emitter native | **✅ DELETED (K4 inc-6B):** the org ladder was inert (chainer derives placement), the byte emitters went native inc-6A (ObjCodeBase/NullInterrupt) + B1 (EndOfRom), debugger.asm re-homed to the game_root stubs. `engine/engine.inc` is gone — the ROM layout is 100% the declared sigil map + registry. |
