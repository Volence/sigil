...
# (the s3k-codepage parcel's script, with this parcel's paths)
set -u
S=/home/volence/sonic_hacks/.scratch/as-squote
A=/home/volence/sonic_hacks/.aeon-squote
TAG="$1"; T="$2"
OUT=$S/aeon/$TAG; rm -rf "$OUT"; mkdir -p "$OUT"
LOG=$S/logs/four-$TAG.log
{
  echo "aeon HEAD: $(git -C "$A" rev-parse HEAD) dirty=$(git -C "$A" status --porcelain --untracked-files=no | wc -l)"
  echo "sigil md5: $(md5sum "$T/release/sigil" | cut -d' ' -f1)"
  "$T/release/sigil" --version | sed -n 1,5p
