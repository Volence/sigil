...
- [tranche 13 SECOND retrospect, 2026-07-13] **`frame_piece_count` shared-helper BLOCKED by the proc-body-splice gap — 2 language gaps discovered** — a rigorous second retrospect pass (loop-until-dry, prompted by Volence checking the loop rigor) caught what the first pass missed: `load_object.emp:79` and `animate.emp:276` both spell `move.w FRAME_PIECE_COUNT(base, off.w), dest` with a BYTE-IDENTICAL "+4 after the bbox bytes" comment (sprites.emp reads FRAME_PIECE_COUNT too, but `(a3)` no-index = weaker match). Attempted (Volence-directed) a `pub comptime fn frame_piece_count(base,off,dest) -> Code` in a new helper-only module `engine.objects.frames` (the aabb.emp pattern — 0 procs, zero bytes, clean dep). BLOCKED by two gaps: **GAP A** — `FRAME_PIECE_COUNT({base}, {off}.w)` fails to parse ("expected `)`, found Dot"): a size-suffixed SPLICED index reg (`{off}.w`) isn't accepted in an asm-template indexed EA, though a LITERAL `(a3,d3.w)` parses fine; workaround `{off}` (default index width) parsed. **GAP B (decisive)** — `{frame_piece_count(a3,d3,d3)}` at statement position in `Load_Object`'s `proc { }` body fails ("expected an instruction, found LBrace"): the statement-position Code-splice is gated to `asm { }` TEMPLATES only (the c79ff6a v1 limitation — `splices_allowed` off in proc bodies). collision.emp/sprites.emp only splice helpers because their consumers are THEMSELVES `comptime fn -> Code { return asm{ … {helper()} … } }` templates, not plain procs. So a plain proc CANNOT adopt a Code-returning comptime-fn helper without being restructured into an asm{}-body proc — a disproportionate rewrite of a clean 40-line proc to centralize ONE instruction. REVERTED to green step-5. THE ASK (step-3(a), significant): (A) accept `{reg}.w`/`{reg}.l` size-suffixed spliced index regs in asm-template EAs; (B) allow statement-position `{code_expr}` splices in PLAIN proc bodies (not just asm{} templates) — this is the gate on the whole step-4 "adopt constructs into existing procs" philosophy: today helpers only compose INTO other asm-templates, so the large corpus of plain procs can't call them. Gap B is the higher-value ask (unblocks helper adoption corpus-wide). — RECORDED (helper blocked; 2 gaps, B is the campaign-relevant one).
