...
- [t19 pass-1 step-4, 2026-07-24] **VDP_DATA/VDP_CTRL hoist EXECUTED (t18 dry-panel B1 row) — consumers pending sweep.** The two port consts + drift ensures now live in `engine.vdp` (pub); bg.emp consumes via `use`. parallax.emp + plane_buffer.emp still carry their local mirrors — the t19 step-6 corpus sweep retrofits them onto the shared consts (byte-neutral; their `_OFF` derives stay local), which CLOSES the t18 row. — RECORDED (hoist done; retrofit at t19 step-6).
- [t19 pass-1 step-4, 2026-07-24] **stop_z80/start_z80 shared-module lift EXECUTED (tranche-5 demand-gated row) — sound_api retrofit pending sweep.** bg.emp was the 2nd consumer; `engine/z80_bus.emp` now owns the templates (kill-list row 36). sound_api.emp still carries its local copies — the t19 step-6 sweep retrofits it (byte-neutral), closing the tranche-5 OPEN row. — RECORDED (lift done; retrofit at t19 step-6).
