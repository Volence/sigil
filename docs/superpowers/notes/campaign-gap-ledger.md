...
- [tranche 11 sprites, 2026-07-11] **Emit_ObjectPieces four-variant unification — a BUILD candidate DEFERRED to Volence** — the four inline flip-variant piece loops (unflipped/xflip/yflip/xyflip, ~160 lines) differ only in Y-transform (simple vs neg+height), X-transform (simple vs neg+width-LUT), tile `eori` mask (`xflip<<11|yflip<<12`), and size-byte source (d1 vs re-read `-6(a3)`). A `comptime fn emit_piece_loop(xflip, yflip) -> Code` would collapse them ~160→~50 lines, BYTE-NEUTRAL (still unrolled — zero-JSR-per-piece preserved). NOT built in-port because: file-specific (no corpus reuse — no other file emits VDP SAT piece loops) + byte-exactness across 4 variants is delicate. Presented at the t11 gate as an optional fast follow-up. — RECORDED (Volence decision at merge)
