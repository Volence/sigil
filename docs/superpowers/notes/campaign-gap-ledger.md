...
- [tranche 17 plane_buffer step-5 charter (row 1066), 2026-07-16] **Probe A ran — Draw_TileColumn measured 7.5%/frame during sustained-max-H, a REAL but ~5× SECONDARY cost; the dominant copy/draw lever is tile_cache's charter. No byte-changing optimization taken this tranche.** LIVE oracle profiler, shipped tip s4.debug.bin (hash-verified 827e18c4), OJZScroll + Debug_Scene_Freeze camera-poke drive (+~16px/f, Camera_X 96→416px), 6-frame sustained-streaming window @ 100% budget. Split (row-1066 charter boundaries): **plane_buffer's charter** — Draw_TileColumn **7.5%** (9587 cyc, ~4800 cyc/call — matches the row-1057 jot) + VInt_DrawLevel drain 2.0% (VBlank). **tile_cache's charter (DOMINANT)** — Tile_Cache_Fill 37.6% (inclusive) = TileCache_FillColumn 35.0% + TileCache_CopyBlockColumn 21.4% (16 calls) + TileCache_DecompressBlock 5.9% + S4LZ_DecompressDict 4.8%. So Draw_TileColumn is real but ~5× smaller than the tile_cache copy/decompress half — the H-crossing residual's lever is tile_cache's, not plane_buffer's. (Caveat: the frozen-scene drive skips EntityWindow_Scan so it UNDER-loads vs real max-H — Lag_Frame_Count stayed 0 with 19.4% headroom; the RELATIVE split is the robust output, not the absolute lag. The ledgered real-max-H lag is 24-27/224 (unified-prefetch A/B).) **NAMED OPTIMIZATION CANDIDATES (plane_buffer's charter, DEFERRED to the coherent H-streaming pass; (a) = my step-5 finding, (b)/(c) = Fable's hot-path second look):** **(a) Draw_TileColumn wrap-split** — the Part-A/B copy loops run a per-word wrap check (`cmpa.l a1,a0 / blo / suba.w #NT_SIZE,a0`) EVERY iteration though the physical-row-59→0 wrap happens ≤ once per column (invariant-ladder: the wrap-check sits below its scope) — hoistable by splitting the loop at the wrap boundary (copy rows-until-wrap unchecked, wrap once, copy remainder), the horizontal analog of Wave-1's FillRow hoist+SR. Est. ~19% of Draw_TileColumn ≈ ~1.4% of frame. **(b) Draw_TileRow_FromCache SEGMENT-SPLIT (Fable)** — the per-cell TRIPLE check (left-clamp / physical-wrap / R-wrap) sits below its scope: each boundary crosses ≤ once per row, so straight-run splitting saves ~15-20 cy/cell × 64 ≈ ~10% of the row path. The V-draw analog of (a) — so the pass covers BOTH draw paths (column + row). **(c) VInt_DrawLevel COLUMN drain as move.l (Fable)** — VDP autoinc applies per WORD access (the ROW drain already relies on this), so a `move.l` writes two column cells per instruction; needs odd-count remainder handling (rows_A/rows_B can be odd). ~300 cy per column drain ≈ ~0.5%/f — small, but VBlank cycles (VBlank-budget lens). ALL THREE DEFERRED to a focused H-streaming perf pass that also tackles tile_cache's DOMINANT half (FillColumn/CopyBlockColumn) — optimizing secondary levers in isolation (byte-changing → re-pin + Probe-B live-verify) is worse sequencing than one coherent pass against real (unfrozen) max-H lag, where Probe B can actually show the win (the frozen drive can't lag). **Step-5 decision: no change this tranche, recorded with the measurement + the named candidates.** — RECORDED (Probe A split; wrap-split (a) + Fable's row-split (b) + move.l column-drain (c); deferred to a coherent H-streaming pass covering both draw paths + tile_cache's dominant half).
