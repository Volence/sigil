...
| Item | Why not |
|---|---|
| `crates/sigil-harness/repin.toml` header — *"resolves every entry against BOTH aeon listings (`s4.lst` / `s4.debug.lst`)"* | **BLOCKED by scope.** `repin.toml` is the byte-mover lane's file and this parcel was told not to touch it. Already recorded as load-bearing in the companion note; it is the sentence that made the pin gate look like an independent instrument. Leaving it is a deferral, not a judgement. |
