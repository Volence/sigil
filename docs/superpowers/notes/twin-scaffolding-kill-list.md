...
| 13 | `aabb_axis_test` template twin — `engine/objects/aabb.inc` (AS macro) vs `engine/objects/aabb.emp` (comptime fn) | rings.asm consumes the .inc AS-side; collision.emp consumes the .emp — the overlap math is duplicated across the seam (the .inc's `utag` param has no .emp twin: hygienic labels) | collision's byte gate pins the .emp expansion against the .inc expansion in the reference region; LOCKSTEP comments in both files | `rings.asm` ports → the .inc's last consumer goes .emp-side; delete the .inc (guard and all). Zero-disp collapse must hold for rings' `(a0)`/`2(a0)` call shapes (probe at that port). |
