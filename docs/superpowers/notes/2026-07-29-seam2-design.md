...
| 5 (mt_bank block) | `song_movingtrucks/drumtest/hcz2.asm`, `movingtrucks_patches/pitchtable*.asm`, `hcz2_patches.asm`, `song_table.asm` | **DONE (seam-2 stage-2c)** — 7 MT `.asm` DELETED; `mt_bank.emp` is canonical (BINCLUDE'd @ $58607 + `mt_syms.asm` supplies SongTable/SongPatchTable). | ✔ whole-ROM byte gate both shapes (`seam2_mt_rom`); `mt_bank.emp` == the `$5BAE8/$5D53A` windows (`mt_port`); the contiguity/co-residency ensures fire on doctored inputs (`mt_negative_probes`, re-baselined to $58607). |
