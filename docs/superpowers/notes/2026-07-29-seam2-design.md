...
|---|---|---|---|
| 5 (dac_samples entry) | `dac_samples.asm` | **DELETE at seam-2** (proposal: move its kill from "Spec 5" earlier — the `.emp` becomes canonical via the emitter). | whole-ROM byte gate both shapes vs e5765873/dab4f06c; `.emp` bank-placement == the `$50000/$58000` reference banks. |
...
| 57 | `dac_sample_tab.asm` | **DELETE at seam-2** (kill condition = "banked-head seam + SND_* comptime source" — §2d supplies the source via co-link). | windowed 90 B oracle → whole-ROM head-table byte gate; the `extern` size guard + the `bankid/winptr/.len` fold match the reference descriptors. |
