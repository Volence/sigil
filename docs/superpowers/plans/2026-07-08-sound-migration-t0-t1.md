...
Executed 2026-07-08 (same day as design), subagent-driven, controller = Fable. Committed code is authoritative over plan snippets. Branches: sigil `sound-migration-t0-t1` (79156bb..9e2bce4, 10 commits, worktree), aeon `sigil-emp-dac` (4782cde, 916f763, c0ef67a).

**Deltas vs the plan (all argued, none silent):**

- **Task 1 (u16le, 79156bb):** `le: bool` threaded through `Ty::Prim`/`Cell::Scalar`/`Cell::Expr` exactly as planned. `self_ref_le` helper mirrors `self_ref_width` for Newtype/Refined traversal (path unreachable today — no u16le-under-newtype customer; documented).
- **Task 1b (b99463f):** one PRE-EXISTING test (`bank_attr_composes_with_cpu_and_vma_in_any_order`) exercised the now-illegal bank:+vma: combination as a positive case (predates DSM.2) — repointed to a vma:-free variant with a NOTE.
- **Task 2 (equ, 63e4ccc):** `equ` reserved CONTEXTUALLY (decl-name positions via `expect_decl_name`), not globally — consistent with the grammar's other contextual keywords; corpus grep proved zero collisions. Equ resolution shares the const memo/cycle stack (same-name const+equ = last-wins, mirroring existing same-named-const behavior; noted in code).
- **Task 3 (c9186af):** ONE commit not two (the headline test inherently couples equ-fold and winptr). Fold runs in `resolve_layout` at convergence (c4, after overlap+bank checks); symbol DEFINITION in `link()` Pass-1b before the single fixup site — satisfies "defined before fixup application" (resolve_layout doesn't thread the table to link). `Value::Typed` wrapping a comptime int accepted as equ value. `Cell::SymRef{windowed}`/BankPtr16* left in place (AS frontend + Z80 backend still produce them). Consequences of winptr→LinkExpr: `zx0(winptr(...))` error code is now `[zx0.arg]` (was `[zx0.symbolic]`; still loud); untyped `data P = winptr(...)` now requires a width annotation (LinkExpr isn't self-widthing; corpus already annotated). **Ledger L7.3 DISCHARGED** — byte-equivalence proven with pre-change pinned bytes, dac exhibit + corpus + full s4 harness identical.
- **Task 4 (0628461): the plan's premise was FACTUALLY WRONG and is corrected here.** The old `BankPtr16Le` apply arm wrote `value as u16` VERBATIM — no window mask at fixup time (the mask always lived in the source-built fixup target tree). So `dw SND_KICK_LEN` would NOT have emitted $857E. The REAL deltas: `db` gains a deferral path (had none — hard frontend error), `dw` compound expressions defer (were hard errors), and out-of-range folds go from silent `as u16` truncation to the loud `[value.out-of-range]` check. The ruling's intent (no silent masking, general tree deferral) holds; the survey confirmed zero callers relied on the old dw path. Judgment call: genuinely-undefined db/dw symbols now surface at LINK, not the frontend converged pass (instruction operands unchanged).
- **Task 5 (baadf0b):** both probes structurally clean, zero workarounds. Probe A descriptor = `0B 00 80 06 00` exactly as designed. Incidental: AS hex is `$X`/`Xh`, not Rust's `{:#X}` (`0X...` is invalid AS syntax).
- **Task 6 (aeon 4782cde):** 24/24 generated files byte-equal via `tools/verify_emit_bin.py`. NO interior labels in any payload (T2/T3 embeds will be clean); `sfx_table.asm` is dc.l-pointer-only, correctly excluded. FLAG for the campaign: `movingtrucks_pitchtable.asm` carries HAND EDITS (SndDefaultPitchTable alias + size assert, commits 5556d76/6c44e46) that `--emit-pitchtable` regeneration clobbers — reconcile generator vs hand edits before T2 touches it.
- **⚠ COLLISION-BIN INCIDENT (mid-session, resolved):** running aeon's tools pytest suite (Task 6 verification) regenerated UNTRACKED, gitignored BINCLUDE inputs — `games/sonic4/data/collision/{heightmaps,heightmaps_rot,angles,solidity}.bin` — with content that DIFFERS from what the reference ROMs bake (latent generator-vs-tree drift, same class as the strlen drift). Both m1d harness tests went red mid-arc; diagnosed via mtime sweep + ROM byte comparison; RESTORED by extracting the original bytes from s4.bin at $2C1FA/$2D1FA/$2E1FA/$2E2FA (HeightMaps/HeightMapsRot/AngleTable/SolidityTable). A re-baseline would have SILENTLY ADOPTED changed collision gameplay data — wrong remedy. **Open item for Volence:** the collision generators' current output ≠ the ROM-baked data; next full tools-suite run re-clobbers. Either intentionally adopt the new collision data (rebuild ROMs + re-baseline) or pin/fix the generator inputs.
- **Task 7 (aeon 916f763 + sigil 089139d):** all 30 SND_* values matched s4.lst exactly; name parity verified mechanically. Test uses the library pipeline not the CLI (embed include_root = module dir; equ values readable off folded `equ_syms`; CLI would warn on module-id/path mismatch). Production-map note: the equs live in a zero-byte `text` carrier section that needs a benign map home at cutover.
- **Task 8 (aeon c0ef67a):** inertness proven bidirectionally; both ROMs sha256-match the PROVENANCE pins before and after the gate edit; sigil's AS frontend parses the new ifndef/else/endif identically (harness green).
- **Task 9 (fb3a612 + 447aa8c):** MIXED ROM BYTE-IDENTICAL, both DEBUG shapes, allowlists m1d-identical (no new entries), lengths 0x658B4/0x673A2. Gap fill $00 verified against reference. Defines threaded via existing `Options.defines`. Review-driven follow-up landed (447aa8c): `assert_rom_matches` + convsym allowlists hoisted to the harness lib (was 4-way duplicated; rule-of-three exceeded) + carrier-emptiness assert (the zero-byte `text` carrier invariant now enforced, not just documented).
- **Task 10 (9e2bce4):** four port-shaped negative probes (straddle via place_sections at $57000; zero-len `[]` blob fires the ensure comptime; emp-equ vs AS-LABEL dup (the realistic migration collision — AS equates don't enter the link table); AS org vs emp region overlap). Each falsified. Note: `org` pins LMA, `phase` only VMA — overlap probes need `org`.

**Reviews:** two-stage (spec + quality) on Tasks 3, 4, 7, 9 — all approved; quality minors logged (defensive-dead-code nits in fold_equ_syms; db/dw rationale comments could trim; guard-message drift risk in the .emp's 10 ensures; AEON_DIR hardcoded default shared with harness convention). Whole-branch adversarial review: see completion handoff.
