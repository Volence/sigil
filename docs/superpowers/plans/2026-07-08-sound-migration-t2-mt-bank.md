...
- [x] **Step 1: Failing test.** For EACH shape (`DEBUG=0`, `DEBUG=1`): lower `aeon/games/sonic4/data/sound/mt_bank.emp` (include_root = its dir, defines = the shape) → place with a map pinning `mt_bank` @ `0x60607` (+ the text carrier region; copy dac_port's map string, swap regions) → inject the `.asm`-side symbol the ensures read → link → assert the flattened `mt_bank` section bytes == the reference window of the matching ROM (`aeon/s4.bin[0x60607..0x63AE8]` plain / `aeon/s4.debug.bin[0x60607..0x6553A]` debug). Reference-gated + `SIGIL_STRICT_GATE` skip pattern, same as `sigil-harness`'s gate idiom. See Execution notes: only ONE cross-seam symbol is actually needed (`MovingTrucks_Bank_Start`), not two — `mt_bank.emp` (as landed in Task 5) never reads `SND_ENGINE_TABLE_BANK` directly.
- [x] **Step 2: RED** (the .emp/map/harness wiring is new) — turned out instant-GREEN (see Execution notes for why, and the falsification pass that substitutes for a real RED). Divergences itemized per DSM.9's stop rule: **none**.
- [x] **Step 3: Commit** — `test(port): mt_port — MT bank .emp region byte-identical to reference, both shapes`
...
- [x] **Step 1: Failing tests** `mixed_mt_rom_matches_assembled_reference` + `mixed_mt_debug_rom_matches_assembled_reference`: assemble the AS side with both gates, lower BOTH `.emp` modules (dac defines-less; mt with `DEBUG` matching the shape), concat sections, one `resolve_layout` + `link`, `assert_rom_matches` vs `ASSEMBLED_LEN`/`DEBUG_ASSEMBLED_LEN` with the same `CONVSYM_REWRITTEN*` allowlists (content identical ⇒ same pins — the module doc-comment reasoning carries over verbatim).
- [x] **Step 2: RED → GREEN.** Instant-GREEN, not RED (see Execution notes — same pattern as Task 6). This is where the R3 deferral + cross-seam resolution proves out end-to-end (`movea.l #SongTable` resolves to `$63AE0`/`$65522` per shape — confirmed by direct byte inspection, see notes). Stop rule: any non-allowlisted diff offset fails loud, itemize per DSM.9 — **zero divergences found**.
- [x] **Step 3:** Full nets: `SIGIL_STRICT_GATE=1 AEON_DIR=… cargo test -p sigil-harness` + `cargo test --workspace` + clippy — ALL green (T1's tests keep passing: DAC-only composition unchanged).
- [x] **Step 4: Commit** — `test(harness): mixed DAC+MT full-ROM byte-identical, both shapes (sound-migration T2 acceptance)`

### Task 8: negative probes (the T1 pattern, commit `9e2bce4`'s sibling)

- [x] In `mt_port.rs` (or beside it): (a) shrink the map region so the section would straddle `$68000` → the bank no-straddle diagnostic fires; (b) inject `SND_ENGINE_TABLE_BANK=0xB` (wrong bank) → the co-residency ensure fires with its message; (c) `-D DEBUG=1` with a 1-entry table via a doctored source string → the length mismatch is a clean error; (d) omit the `DEBUG` define → clean unknown-name error naming `DEBUG`.
- [x] GREEN + commit — `test: T2 negative probes — straddle/wrong-bank/table-len/missing-define all loud`
...
### Task 6 (`mt_port.rs` region-level byte gate) — DONE (sigil commit, branch `sound-migration-t2`)

- **File:** `crates/sigil-cli/tests/mt_port.rs` (new), `dac_port.rs`'s structure mirrored: a `sound_dir()` (`AEON_DIR`-honoring, same default), a `map_toml()` (two regions: `text` nominal-carrier + `mt_bank` @ `0x60607` size `0x79F9`), a `compile_real_file(debug)` driving parse → lower → `place_sections` → link, and two `#[test]`s.
- **Only ONE cross-seam symbol needed, not two — a plan-brief correction.** The Step-1 brief (written before Task 5 landed) said to inject `MovingTrucks_Bank_Start=0x60000` AND `SND_ENGINE_TABLE_BANK=0xC`. Reading the REAL `mt_bank.emp` (Task 5's Deviation 2, already recorded) shows all five ensures were rewritten to `bankid("X") == bankid("MovingTrucks_Bank_Start")` — `SND_ENGINE_TABLE_BANK` never appears in the file at all. Confirmed empirically with a throwaway probe test (`lower_module` + dump of `module.link_asserts`) before writing the real assertions: all 5 `LinkAssert`s reference only `Sym("MovingTrucks_Bank_Start")` on their RHS (LHS is one of `Song_MovingTrucks`/`MovingTrucks_Patches`/`Song_DrumTest`/`Song_HCZ2`/`HCZ2_Patches`). The test supplies exactly one synthetic symbol.
- **Cross-seam symbol composition technique:** `ports.rs::probe_b`'s `phase` idiom, applied directly — `as_bank_start_label()` assembles `"cpu 68000\nphase $60000\nMovingTrucks_Bank_Start:\n\tdc.w 0\n"` via `sigil_frontend_as::assemble`, giving one AS `Section` whose `vma_base == Some(0x60000)` (so `bankid("MovingTrucks_Bank_Start")` folds to `0x60000 >> 15 == 0xC` at link, matching the real equ). That section is then stamped `lma = 0x0100_0000` / `placement = Pinned` / `group = None` by hand (mirroring `place_sequential`'s bookkeeping for a single section) — a harness-private LMA far outside both map regions (`text` @ `0x0..0x10`, `mt_bank` @ `0x60607..0x68000`), so appending it to the `.emp`'s already-`place_sections`-placed sections before ONE `resolve_layout`+`link` pass cannot collide on LMA. `place_sections` itself is NOT used for this section (it requires every section to match a map region by name, and this synthetic section is a harness fixture with no map entry) — the same reasoning `probe_b` used for its own no-map `place_sequential` tail.
- **`check_link_asserts` called explicitly** (not implied by `link`) exactly as `probe_b` does, against `module.link_asserts` (captured from `lower_module`'s return before `place_sections` consumes `module.sections`) and the POST-`resolve_layout` sections — asserted `.iter().all(|d| d.level != Error)` in both tests, satisfying "assert the five ensures PASSED (link succeeded)."
- **Map shape, confirmed by a throwaway section-dump probe before writing the real test:** `mt_bank.emp`'s top-level `ensure`s (`SONG_COUNT < $FF` and the `MT_PITCHTAB_OFFSET` detune guard) DO open the lazy default `text` carrier (`Item::Ensure` calls `ensure_default`, per `lower/mod.rs`) — and since the module's OTHER top-level ensures are the five cross-seam ones written AFTER the `section mt_bank {}` block closes, lowering actually produces **three** sections in order: `text` (0 bytes, before `mt_bank`), `mt_bank` (the real payload), `text` again (0 bytes, after). Both `text` instances share one map region cumulatively (R7/P5's proven per-region cumulative cursor — both land at the region's base, zero-length, harmless). `image_len` for `mt_bank` probed as exactly 13,537 bytes (DEBUG=0) and 20,275 bytes (DEBUG=1) — matching the fact base's plain/debug tail arithmetic (`0x60607+13537=0x63AE8`, `0x60607+20275=0x6553A`) before the reference-ROM diff was ever run, i.e. confirmed twice independently.
- **RED / falsification evidence (DSM.9, since the real RED was instant-green):** both tests passed on the FIRST run with no fixes (`cargo test -p sigil-cli --test mt_port` → 2 passed immediately) — the `.emp` was pre-verified in Task 5 and every mechanism `mt_port.rs` needs (place_sections, cross-seam phase-label composition, `check_link_asserts`) was already proven by Task 2's probes / `ports.rs::probe_b` / `dac_port.rs`. Per the task's TDD-loose instruction, falsified once instead: temporarily XOR'd the first expected byte of the plain-shape reference window (`expected[0] ^= 0xFF`) and re-ran `mt_bank_region_matches_reference` alone — it failed loudly with `mt_bank (DEBUG=0) vs s4.bin[0x60607..0x63AE8]: first diff at offset 0x0 … candidate: [03, 38, b7, …] expected: [fc, 38, b7, …]` (`0x03 XOR 0xFF == 0xFC`, confirming the REAL computed byte is `0x03`, and the diff-reporting path — length check, first-diff scan, 16-byte context window on both sides — genuinely fires against live linked bytes, not a hardcoded pass). Reverted immediately after capturing the panic output; the file that landed has no trace of the probe.
- **Both tests, GREEN:** `mt_bank_region_matches_reference` (DEBUG=0 vs `s4.bin[0x60607..0x63AE8]`, 13,537 bytes) and `mt_bank_debug_region_matches_reference` (DEBUG=1 vs `s4.debug.bin[0x60607..0x6553A]`, 20,275 bytes) — both byte-identical, zero divergence (DSM.9 stop rule: itemize-and-fail — nothing to itemize).
- **Full nets:** `SIGIL_STRICT_GATE=1 AEON_DIR=/home/volence/sonic_hacks/aeon cargo test -p sigil-cli --test mt_port` — 2 passed. `cargo test --workspace` — every crate green (112 `test result: ok` blocks, 0 failed, exit 0). `cargo clippy --workspace --all-targets -- -D warnings` — clean, zero warnings.
- No BLOCKED conditions; no aeon files or production sigil code touched — only the new test file (`git status --short` shows exactly one untracked path, `crates/sigil-cli/tests/mt_port.rs`).

### Task 7 (the mixed full-ROM gate, DAC+MT) — DONE (sigil commit, branch `sound-migration-t2`)

- **Helper approach chosen:** a SIBLING function, `assemble_mixed_mt_as_side` (`crates/sigil-harness/src/lib.rs`), not a grown parameter on `assemble_mixed_dac_as_side` — keeps the T1 helper's signature (and its two existing callers, the unchanged T1 tests) completely untouched, matching the task's "keep the T1 helper's callers compiling unchanged" instruction literally rather than via a default-argument workaround. The sibling defines `SOUND_DRIVER_ENABLED` + `SIGIL_EMP_DAC` + `SIGIL_EMP_MT` (+ `__DEBUG__` when `debug`), mirroring the existing function's body shape exactly (same `Options`/`assemble_root` call, same `Result<Module, String>` return so the T2 test composes it identically to the T1 one).
- **Test-file helper refactor:** `placed_emp_sections` (T1, unchanged behavior) was factored to share a body (`placed_module_sections`) with a new `placed_emp_sections_with_mt`, so both `dac_samples.emp` and `mt_bank.emp` go through the same parse→lower→place→zero-byte-carrier-check pipeline, parameterized by module filename, `defines`, and map string. A new `emp_bank_map_with_mt()` extends `emp_bank_map()`'s three regions with a fourth, `mt_bank` @ `0x60607` size `0x79F9` (mt_port.rs's R7 region, copied verbatim). `build_mixed_mt_rom` is the T2 sibling of `build_mixed_rom`: assembles the AS side via `assemble_mixed_mt_as_side`, composes AS + both placed `.emp` modules' sections, does ONE `resolve_layout`+`link`, and ADDITIONALLY captures `mt_bank.emp`'s `link_asserts` from a second lightweight parse+lower pass (mirroring how `mt_port.rs`'s `compile_real_file` captures them before `place_sections` consumes `module.sections`) so `check_link_asserts` can run explicitly and its diagnostics can be returned to the test for an explicit "must all pass" assertion — this is the "mirror how mt_port.rs calls check_link_asserts if the mixed path doesn't already" instruction; the mixed composition path does NOT call it as part of ordinary `link()`, so it is called explicitly here, matching mt_port.rs's pattern precisely.
- **Both new tests, GREEN on first run — instant-green, not RED** (same shape as Task 6): `mixed_mt_rom_matches_assembled_reference` and `mixed_mt_debug_rom_matches_assembled_reference` both passed immediately on first compile+run. Root cause (as with Task 6): every mechanism this task needs was already independently proven — the AS-side gate composition by T1's `assemble_mixed_dac_as_side`/`mixed_dac_rom.rs`, the `.emp`-side placement+link by `mt_port.rs` (Task 6), the imm32 deferral by Task 3's `imm32_defer.rs` net (which already re-ran the FULL `sigil-harness` reference gates and stayed green), and the dual-`text`-carrier tolerance by Task 2's P5 probe. Composing all four in one test exercised no new code path, only new WIRING of already-proven paths.
- **Falsification (DSM.9, since there was no real RED to record) — TWO separate falsifications, each reverted immediately after capturing evidence:**
  1. **Byte-diff detector:** in the plain-shape test, temporarily XOR'd `rom[0x60607]` (the first byte of `mt_bank`'s window) with `0xFF` before `assert_rom_matches`. Failed loudly: `DSM.9 STOP: mixed MT ROM diverges from the reference at 1 unexpected offset(s); FIRST at 0x60607 (DSM.9 STOP: mixed MT 0xfc != ref 0x03)` — `0x03 XOR 0xFF == 0xFC`, confirming the real computed byte is `0x03` (matching `s4.lst`'s `Song_MovingTrucks` first byte) and that the diff-reporting path (length check, first-diff scan, allowlist filter, 16-byte context) fires against genuinely live composed bytes, not a hardcoded pass.
  2. **Ensure-check assertion:** appended a synthetic always-`Error` `Diagnostic` (`"FALSIFICATION PROBE: synthetic always-fail ensure"`, `SourceId(0)`/span `0..0`) to the real (5-element, all-passing) `assert_diags` Vec returned by `build_mixed_mt_rom`, before the `assert!(assert_diags.iter().all(...))` line. Failed loudly with the panic message containing exactly that synthetic diagnostic — confirming the "must all PASS" assertion genuinely inspects the returned Vec's contents rather than trivially passing on an unused/ignored value. A separate one-line `eprintln!("PROBE link_asserts.len() = {}", link_asserts.len())` probe (also reverted) confirmed the captured `Vec<LinkAssert>` has exactly 5 entries every run — the five `bankid(...) == bankid("MovingTrucks_Bank_Start")` co-residency ensures from `mt_bank.emp`, genuinely executed by `check_link_asserts` and genuinely passing (empty diagnostic list) in both shapes.
  Both falsifications reverted immediately after capturing their panic output; `git diff` against the landed commit shows no trace of either probe.
- **The movea.l imm32 deferral resolving end-to-end — direct byte evidence** (beyond the full-ROM assertion passing): looked up the two `sound_api.asm` cross-seam sites in the reference listings/binaries directly. Plain shape: `s4.lst:24531` `83/ 5DC6: 207C 0006 3AE0  movea.l #SongTable, a0` and `s4.lst:24547` `99/ 5DEC: 207C 0006 3AE4  movea.l #SongPatchTable, a0` — confirmed via `xxd -s 0x5DC6 -l 8 s4.bin` / `xxd -s 0x5DEC -l 8 s4.bin` to be `207c 0006 3ae0 …` / `207c 0006 3ae4 …` in the reference itself. Debug shape: `s4.debug.lst:54099`/`:54115` at `7284`/`72AA`, values `0006 5522` / `0006 552E`; `xxd` on `s4.debug.bin` at those offsets confirms the same bytes. Since both full-ROM tests assert byte-identity against exactly these reference files and PASS, the composed (sigil-built) ROM reproduces these exact opcode+imm32 bytes too — i.e. the `movea.l #SongTable,a0`/`movea.l #SongPatchTable,a0` instructions, assembled by the AS frontend with `SongTable`/`SongPatchTable` UNRESOLVED at AS-time (Task 3's `Value32Be` deferral is what let them assemble at all), resolved through the ONE joint `resolve_layout`+`link` pass to `$63AE0`/`$63AE4` (plain) and `$65522`/`$6552E` (debug) — exactly the `.emp`-defined `mt_bank.emp` `SongTable`/`SongPatchTable` label addresses per the fact base. This is the concrete "R3 deferral + cross-seam resolution proving out end-to-end" the plan's Step 2 asked for.
- **Module doc-comment extended** (`mixed_dac_rom.rs`, per the task instruction): the module doc now covers both tranches (renamed header, "T1+T2 acceptance"), documents the fourth map region and the dual-`.emp`-module composition, adds a "T2 adds NO new gap" note — the MT `.asm` else-arm's `org` resume lands EXACTLY at `mt_bank.emp`'s section end (`$63AE8`/`$6553A`, matching the fact base's tail addresses), unlike the DAC banks' `align $8000` gaps, because `mt_bank.emp`'s items emit contiguously (§4.3 no-auto-pad) all the way to `SongPatchTable_End` with the SFX block picking up immediately after — and a new "Cross-seam resolution (T2)" section spelling out the imm32-deferral + real (non-synthetic) `MovingTrucks_Bank_Start` distinction from `mt_port.rs`.
- **Divergences found: NONE** (DSM.9 stop rule — itemize-and-fail; nothing to itemize). Both shapes byte-identical to `aeon/s4.bin`/`aeon/s4.debug.bin` modulo the same `CONVSYM_REWRITTEN`/`CONVSYM_REWRITTEN_DEBUG` allowlists T1 already used (content identical ⇒ same convsym/fixheader-rewritten offsets, confirmed by `assert_rom_matches`'s own "allowlisted bytes must genuinely differ" check passing too).
- **Full nets, all green:** `SIGIL_STRICT_GATE=1 AEON_DIR=/home/volence/sonic_hacks/aeon cargo test -p sigil-harness` — every test file green, including `m0_regions`, `m1b_gate` (5 tests), `m1c_vector_table`, `m1d_rom`, `m1d_debug_rom`, and all 4 `mixed_dac_rom.rs` tests (T1's 2 unchanged + T2's 2 new). `cargo test --workspace` — 112 `test result: ok` blocks, 0 failed. `cargo clippy --workspace --all-targets -- -D warnings` — clean, zero warnings, exit 0.
- **Files changed:** `crates/sigil-harness/src/lib.rs` (+32 lines: the new `assemble_mixed_mt_as_side` sibling); `crates/sigil-harness/tests/mixed_dac_rom.rs` (extended module doc + 2 new map/composition helpers + 2 new tests). No aeon files, no other production sigil code touched.
- No BLOCKED conditions.

### Task 8 (negative probes) — DONE (sigil commit, branch `sound-migration-t2`)

- **File:** `crates/sigil-cli/tests/mt_negative_probes.rs` (new, sibling of `mt_port.rs` — matches T1's own organization: `sound_migration_negative_probes.rs` is a sibling file to `dac_port.rs`, not folded into it, so this file follows the same precedent rather than growing `mt_port.rs`). 4 tests, all instant-green.
- **(a) straddle** — `straddle_doctored_map_base_is_a_loud_bank_boundary_error`: the REAL `mt_bank.emp` (DEBUG=0), placed via a doctored map (`mt_bank` region `lma_base = 0x67000` instead of the real `0x60607` — NOT `$8000`-aligned, close enough to `$68000` that the 13,537-byte section straddles it). `resolve_layout` returns `Err` naming `` section `mt_bank` [0x67000, 0x6A4E1) straddles the 0x68000 bank boundary (0x8000-byte bank) ``; asserted `.message.contains("mt_bank") && .message.contains("straddle")`.
- **(b) wrong-bank ensure** — `wrong_bank_cross_seam_label_fires_all_five_co_residency_ensures`: the real `mt_bank.emp` placed correctly at `$60607` (bank `$C`), but the synthetic cross-seam `MovingTrucks_Bank_Start` label supplied at `$58000` (bank `$B`) instead of the real `$60000` (also bank `$C`). `resolve_layout`/`link` both still succeed (only the ensures should fail — asserted); `check_link_asserts` returns all 5 diagnostics, all `Level::Error`, asserted against two exact message substrings verbatim from the file: `"not co-located with the engine-table bank"` (Song_MovingTrucks) and `"MT patch bank not co-located"` (MovingTrucks_Patches).
- **(c) table-length mismatch, choice made:** Task 2's P3 (`lower_data.rs::p3_mismatched_array_length_against_const_n_is_clean_error_not_panic`) already pins the bare generic shape byte-for-byte identical to the task brief's sketch — re-running it verbatim would be pure duplication. Chose the MT-COMPOSITION-CONTEXT variant per the task's own judgment clause: an inline module reproducing `mt_bank.emp`'s actual TWO-PARALLEL-TABLE shape (`SongTable`/`SongPatchTable` sharing one `SONG_COUNT`), with only `SongPatchTable` doctored to 2 elements against `SONG_COUNT=3`. `table_length_mismatch_in_mt_style_dual_table_composition_is_clean_error`: fires the clean diagnostic `"array length mismatch: expected 3 element(s), got 2"` — same diagnostic class as P3, but proves the MT-specific compositional risk (a hand-edit to one of the two twinned tables, forgetting the other) rather than a single isolated array.
- **(d) missing define** — `missing_debug_define_is_a_clean_unknown_name_error_not_a_panic`: the real `mt_bank.emp` lowered with `defines: vec![]` (no `-D DEBUG=` at all). Fires 8 separate `` unknown name `DEBUG` `` `Level::Error` diagnostics (one per `DEBUG` reference site: `SONG_COUNT`'s if-expression plus the three DEBUG-gated data items' `if`/pad conditionals), no panic; asserted `.message.contains("unknown name") && .message.contains("DEBUG")` on at least one.
- **(e) detune guard — SKIPPED, redundant with Task 2's P4** (documented inline in the file's header comment and above probe (d)): P4 (`lower_data.rs::p4_ensure_len_against_pinned_const_fires_loud_message_when_unequal`) already pins the EXACT mechanism `mt_bank.emp`'s own `MT_PITCHTAB_OFFSET` detune ensure uses — a comptime `embed(...).len` vs. a plain pinned `const` via `ensure(...)`, firing the interpolated message on mismatch. Judged (per the task's own escape clause) that there is no MT-specific compositional wrinkle here (unlike (c)'s genuine dual-table interaction) — the detune guard is a single scalar comparison with zero interaction with `SONG_COUNT`/DEBUG-gating/bank structure, so a second copy under a different const name would add zero coverage.
- **Map-region fix needed for (a)/(b):** both probes' first attempt failed with `` section `text` has no region in the map `` — `mt_bank.emp`'s top-level `ensure`s open the lazy zero-byte default `text` carrier (P5/R7, TWO instances per module lowering), so both doctored maps needed the same `text` region `mt_port.rs::map_toml` carries, not just `mt_bank`. Fixed by copying that region into both maps unchanged.
- **Falsification, ALL FOUR probes (per the task's "falsify at least one" — did all four for full confidence given three of four assert on comptime/link mechanisms new to this file):**
  1. (a) restored `lma_base` to the real `$60607` (aligned, no straddle) and flipped to `.expect_err(...)` unchanged (i.e. kept expecting an error at the now-correct base) — panicked with the genuine `Ok(...)` value printed (a real placed+resolved section list), confirming `resolve_layout` truly returns `Ok` at the real base and the straddle-`Err` in the landed test is not a tautology. Reverted.
  2. (b) restored the cross-seam label to the real `$60000` and kept the `assert_eq!(assert_diags.len(), 5, ...)` — failed with `left: 0, right: 5` (i.e. `assert_diags` was genuinely `[]` at the real address). Reverted.
  3. (c) flipped the doctored (2-vs-3) module's assertion to expect clean lowering — failed, printing the real `"array length mismatch: expected 3 element(s), got 2"` diagnostic. Reverted.
  4. (d) kept `defines: vec![]` but flipped the assertion to expect clean lowering — failed, printing the real 8-diagnostic `` unknown name `DEBUG` `` list. Reverted.
