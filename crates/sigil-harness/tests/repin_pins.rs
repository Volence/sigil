...
fn generated_pins_match_the_hand_typed_baseline() {
    // input-6button wave (2026-08-02): the full 6-button burst rewrite grows the
    ...
    assert_eq!(pins::ANIMATE.plain_base, 0x3014);  // −0x10 PAL NTSC-only wave
    assert_eq!(pins::ANIMATE.debug_base, 0x35F2);  // −0x10 PAL NTSC-only wave
    ...
    // the upstream wave.
    assert_eq!(pins::RINGS.plain_base, 0x339E);  // −0x10 PAL NTSC-only wave
    assert_eq!(pins::RINGS.debug_base, 0x3AA2);  // −0x10 PAL NTSC-only wave
    ...
    assert_eq!(pins::CORE.plain_base, 0x2910);  // −0x10 PAL NTSC-only wave
    assert_eq!(pins::CORE.plain_len, 0x2E4);    // −0xA c4 Spawn_Count store removals
    assert_eq!(pins::CORE.debug_base, 0x2AA6);  // −0x10 PAL NTSC-only wave
    assert_eq!(pins::CORE.debug_len, 0x72C);    // −0xA c4 Spawn_Count store removals
    assert_eq!(pins::DPLC.plain_base, 0x2860);  // −0x10 PAL NTSC-only wave
    assert_eq!(pins::DPLC.debug_base, 0x2A02);  // −0x10 PAL NTSC-only wave
    assert_eq!(pins::DPLC.plain_len, 0xB0);     // +0xC: item-11 bcs + post-loop commit (both procs)
    ...
    assert_eq!(pins::DELETE_OBJECT, pins::Pin { plain: 0x29E0, debug: 0x2B76 });  // −0x10 PAL NTSC-only wave
...
fn secondary_pin_classes_match_the_hand_typed_baseline() {
    ...
    assert_eq!(pins::SOUND_API.debug_base, 0x7F30);  // +0x8C net, t24
