...
fn generated_pins_match_the_hand_typed_baseline() {
    ...
    assert_eq!(pins::BOOT.plain_len, 0x198);  // -0x8 R6: `end` re-spelled from the neighbour's head to section:boot, so the pin now measures this region's own extent instead of the gap to its neighbour. Length only; the base is untouched.  // blanket-restore: UNCHANGED — the −8 B ori.l deletion fits inside the existing 16-align pad ahead of BOOT_DATA  // +0x8 item27: EntryPoint's `lea (SYSTEM_STACK).w, sp` (4 B) + a 4 B align pad the new plain length needs
    ...
    assert_eq!(pins::BOOT_DATA, pins::Pin { plain: 0x398, debug: 0x3A0 });  // alignment-flip: plain -0x8, debug HOLDS — the walk rounds boot_data to its DECLARED word alignment, not the 16-quantum inferred from the pinned base's residue; plain's boot ends at 0x398 (0x200+0x198) and the 8 B was pad, debug's boot already ends 16-aligned at 0x3A0 so it never carried any  // debug -0x10 blanket-restore: rides BOOT.debug_len; both shapes converge again  // +0x8/+0x4 item27: rides BOOT's growth; both shapes now start boot_data at the same address  // debug +0x10 art-streaming-p2-task3: rides BOOT.debug_len's boot-debug reach ripple
    ...
    assert_eq!(pins::ANIMATE.plain_base, 0x33D6);  // +0x2 ls12-refreeze (chain 204, aeon 483b3e12 -> 157e59bf absorbed in one step): the +0xC boot slide (the resident Z80 blob's even-rounded length 0x1814 -> 0x1820, inherited by everything downstream of boot) less CORE's own -0xA in the same span  // -0x3e alignment-flip: CORE's -0x30 plus core's tail pad 8 and sprites' tail pad 6, all quantum pad the walk no longer inserts (each section packs flush to its declared WORD alignment instead of the 16/8-quantum inferred from its pinned base); pure slide, len holds  // +0x60 parcel-r1-splice  // +0x20 offscreen-ship-setreg-replay  // +0x40 offscreen-frame-top-ship  // +0x10 parcel-w: the object family takes the PLAIN-only slide from buffers.emp growing 0x10 (the HScroll DMA-length key learned about pcfg_anchor_ch). The debug shape absorbs it in existing alignment slack, which is why only plain moves.  // -0x30 blanket-restore (vdp_init -0x10 + hblank -0x20)  // -0xE0 effects-module-split  // +0xF0 effects-p1-raster  // +0x10 item27: the boot-growth slide (aligned), which every region downstream of boot inherits  // +0x30 defect-batch-8  // +0x10 sst-fold  // +0x80 art-streaming-p2-task2  // +0x30 art-streaming-p2-task3  // +0x20 art-streaming-p2-task4  // +0x20 art-streaming-p2c-t8-t9  // -0x50 wave-f3-f1-f6  // +0x140 sound-pkg1  // -0x60 sound-pkg4  // +0x160 aeon-arctan  // +0x10 replay-hash-layout-proof  // +0x10 character-lens-sweep // -0x10 effects-p3-parcel-c2 (the three legacy per-field installers deleted from the effects modules; every region downstream inherits the slide) // -0x50 delete-percell-hscroll (BUFFERS plain_len 0x330->0x2E0: Parallax_Fill_PerCell/Static_Hscroll_Cell fill + DMA and the mode-key select deleted in buffers.emp; the object family sits downstream of buffers and upstream of parallax, so it inherits ONLY the buffers slide)
    assert_eq!(pins::ANIMATE.debug_base, 0x3C26);  // +0xC ls12-refreeze: the boot slide alone; CORE's debug length holds  // -0x2e alignment-flip: CORE's debug -0x1e plus the 0xE pad between core and sprites and core's own -2 (the relaxed jbsr Draw_Sprite, see CORE.debug_len); sprites packs flush to animate in both shapes  // +0x60 parcel-r1-splice  // +0x10 offscreen-ship-setreg-replay  // +0x50 offscreen-frame-top-ship  // +0x40 blanket-restore-r2 (vdp_init +0x50 for the Set_VDP_Reg bound assert, vblank -0x10 for the dead $8F02 write; DEBUG only)  // -0x40 blanket-restore (boot -0x10 + vdp_init -0x10 + hblank -0x20)  // +0x10 effects-p2-palette (debug-only upstream align step)  // -0xF0 effects-module-split  // +0xF0 effects-p1-raster  // +0x10 item27: same boot-growth slide  // +0x4c defect-batch-8  // +0x10 sst-fold  // +0x80 art-streaming-p2-task2  // +0x40 art-streaming-p2-task3  // +0x10 art-streaming-p2-task4  // +0x30 art-streaming-p2c-t8-t9  // -0x50 wave-f3-f1-f6  // +0x140 sound-pkg1  // -0x60 sound-pkg4  // +0x160 aeon-arctan  // +0x10 replay-hash-layout-proof  // +0x20 character-lens-sweep // +0x10 raster-substrate-byte-moving: buffers.emp grew for the off-screen ship guard's DERIVED palette line (item 4), and the object family sits downstream of buffers. DEBUG only — plain absorbs it in existing alignment slack. // -0x60 delete-percell-hscroll (BUFFERS debug_len 0x338->0x2D8: same deletion, the debug shape gives back 0x10 more because its per-cell fill carried the DEBUG-fenced assert) // +0x46 sprite-owner: sprites.emp grew 0x46 ahead of AnimateSprite (Sprite_Owner's long-wise clear plus the owner_term stamp at the three SAT writers). The object family is downstream. DEBUG only. // +0x32 dma-straddle-counter: DEBUG-only instrumentation upstream — dma_queue debug_len 0x338->0x346 (+0xE, Dbg_DMA_Straddle_All) and vblank debug_len 0x1E8->0x20C (+0x24, DMA_Peak_Important). The object family is downstream of both and takes the sum as a pure slide: every debug region from HBLANK to ANIMATE moves +0x32 with no length change, and the plain shape does not move at all.
    ...
    // the upstream wave.
    assert_eq!(pins::RINGS.plain_base, 0x3870);  // +0x108 ls12-refreeze: ANIMATE's +0x2 plus COLLISION plain_len +0x106 (the Game.solid_pushed / Game.spring_launched hook sites and the rest of collision.emp's growth across the ~360 aeon commits this step absorbs; not bisected per aeon commit here)  // -0x3e alignment-flip: the ANIMATE slide, inherited (animate/collision/rings were already packing flush, so nothing extra between them)  // +0x60 parcel-r1-splice  // +0x20 offscreen-ship-setreg-replay  // +0x40 offscreen-frame-top-ship  // +0x10 parcel-w: same buffers.emp plain-only slide as ANIMATE.  // -0x30 blanket-restore  // -0xE0 effects-module-split  // +0xF0 effects-p1-raster  // +0x10 item27: boot-growth slide  // +0x30 defect-batch-8  // +0x10 sst-fold  // +0x80 art-streaming-p2-task2  // +0x30 art-streaming-p2-task3  // +0x20 art-streaming-p2-task4  // +0x20 art-streaming-p2c-t8-t9  // -0x50 wave-f3-f1-f6  // +0x140 sound-pkg1  // -0x60 sound-pkg4  // +0x160 aeon-arctan  // +0x10 replay-hash-layout-proof  // +0x10 character-lens-sweep // -0x10 effects-p3-parcel-c2 (the three legacy per-field installers deleted from the effects modules; every region downstream inherits the slide) // -0x50 delete-percell-hscroll (BUFFERS plain_len 0x330->0x2E0: Parallax_Fill_PerCell/Static_Hscroll_Cell fill + DMA and the mode-key select deleted in buffers.emp; the object family sits downstream of buffers and upstream of parallax, so it inherits ONLY the buffers slide)
    assert_eq!(pins::RINGS.debug_base, 0x41EC);  // +0x112 ls12-refreeze: ANIMATE's +0xC plus COLLISION debug_len +0x106  // -0x2e alignment-flip: the ANIMATE debug slide, inherited  // +0x60 parcel-r1-splice  // +0x10 offscreen-ship-setreg-replay  // +0x50 offscreen-frame-top-ship  // +0x40 blanket-restore-r2 (same flat debug slide as ANIMATE; plain holds)  // -0x40 blanket-restore  // +0x10 effects-p2-palette (debug-only upstream align step)  // -0xF0 effects-module-split  // +0xF0 effects-p1-raster  // +0x10 item27: boot-growth slide  // +0x4c defect-batch-8  // +0x10 sst-fold  // +0x80 art-streaming-p2-task2  // +0x40 art-streaming-p2-task3  // +0x10 art-streaming-p2-task4  // +0x30 art-streaming-p2c-t8-t9  // -0x50 wave-f3-f1-f6  // +0x140 sound-pkg1  // -0x60 sound-pkg4  // +0x160 aeon-arctan  // +0x10 replay-hash-layout-proof  // +0x20 character-lens-sweep // +0x10 raster-substrate-byte-moving: buffers.emp grew for the off-screen ship guard's DERIVED palette line (item 4), and the object family sits downstream of buffers. DEBUG only — plain absorbs it in existing alignment slack. // -0x60 delete-percell-hscroll (BUFFERS debug_len 0x338->0x2D8: same deletion, the debug shape gives back 0x10 more because its per-cell fill carried the DEBUG-fenced assert) // +0x46 sprite-owner: the same sprites.emp slide ANIMATE takes, inherited from upstream. DEBUG only. // +0x32 dma-straddle-counter: same DEBUG-only upstream slide as ANIMATE above (dma_queue +0xE, vblank +0x24); no length change, plain does not move.
    ...
    assert_eq!(pins::RINGS.plain_len, 0x1BE);   // +8 ring-sparkle: the Game.ring_collected call site  // −6: item 10 DrawRings fold
    assert_eq!(pins::RINGS.debug_len, 0x224);   // +8 ring-sparkle: same call site, same size in both shapes // +0xA sprite-owner: the DEBUG-fenced owner stamp sits INSIDE DrawRings - move.w d5,d0 (2) + add.w d0,d0 (2) + move.w #1,(a6,d0.w) (6). Inside the region, so the length moves and the base does not.
    ...
    assert_eq!(pins::CORE.plain_base, 0x2CCE);  // +0xC ls12-refreeze: the boot slide  // -0x30 alignment-flip: the quantum pads removed upstream, summed (boot_data 8, the post-blob boot tail 6, vdp_init 2, sprite_table 10, vblank 8, s4lz 8, dplc 2, core's own 4 = 0x30); every one of those sections is declared WORD and now packs flush  // +0x60 parcel-r1-splice  // +0x20 offscreen-ship-setreg-replay  // +0x40 offscreen-frame-top-ship  // +0x10 parcel-w: same buffers.emp plain-only slide as ANIMATE.  // -0x30 blanket-restore  // -0xE0 effects-module-split  // +0xF0 effects-p1-raster  // +0x10 item27: boot-growth slide  // +0x10 defect-batch-8  // +0x10 sst-fold  // +0x80 art-streaming-p2-task2  // +0x30 art-streaming-p2-task3  // +0x20 art-streaming-p2-task4  // +0x20 art-streaming-p2c-t8-t9  // -0x50 wave-f3-f1-f6  // +0x140 sound-pkg1  // -0x60 sound-pkg4  // +0x160 aeon-arctan  // +0x10 replay-hash-layout-proof  // +0x10 character-lens-sweep // -0x10 effects-p3-parcel-c2 (the three legacy per-field installers deleted from the effects modules; every region downstream inherits the slide) // -0x50 delete-percell-hscroll (BUFFERS plain_len 0x330->0x2E0: Parallax_Fill_PerCell/Static_Hscroll_Cell fill + DMA and the mode-key select deleted in buffers.emp; the object family sits downstream of buffers and upstream of parallax, so it inherits ONLY the buffers slide)
    assert_eq!(pins::CORE.plain_len, 0x2EE);  // -0xA ls12-refreeze: core.emp shrank across the aeon span 483b3e12..157e59bf (not bisected per aeon commit here; the debug length holds)  // -0x8 R6: `end` re-spelled from the neighbour's head to section:core, so the pin now measures this region's own extent instead of the gap to its neighbour. Length only; the base is untouched.    // addrfree-invariant: plain's +0x40 span is ≡0 (mod 4), tail pad unchanged  // +0x18 defect-batch-8
    assert_eq!(pins::CORE.debug_base, 0x2FB2);  // +0xC ls12-refreeze: the boot slide  // -0x1e alignment-flip: the debug ladder's pads, summed (boot_data holds at 0x3A0; post-blob tail 4, dma_queue 2, hblank 4, game_loop 2, s4lz 4, math 8, dplc 2, core's own 4 = 0x1e)  // +0x60 parcel-r1-splice  // +0x10 offscreen-ship-setreg-replay  // +0x50 offscreen-frame-top-ship  // +0x40 blanket-restore-r2 (same flat debug slide; plain holds)  // -0x40 blanket-restore  // +0x10 effects-p2-palette (debug-only upstream align step)  // -0xF0 effects-module-split  // +0xF0 effects-p1-raster  // +0x10 item27: boot-growth slide  // +0x20 defect-batch-8  // +0x10 sst-fold  // +0x80 art-streaming-p2-task2  // +0x40 art-streaming-p2-task3  // +0x10 art-streaming-p2-task4  // +0x30 art-streaming-p2c-t8-t9  // -0x50 wave-f3-f1-f6  // +0x140 sound-pkg1  // -0x60 sound-pkg4  // +0x160 aeon-arctan  // +0x10 replay-hash-layout-proof  // +0x20 character-lens-sweep // +0x10 raster-substrate-byte-moving: buffers.emp grew for the off-screen ship guard's DERIVED palette line (item 4), and the object family sits downstream of buffers. DEBUG only — plain absorbs it in existing alignment slack. // -0x60 delete-percell-hscroll (BUFFERS debug_len 0x338->0x2D8: same deletion, the debug shape gives back 0x10 more because its per-cell fill carried the DEBUG-fenced assert) // +0x32 dma-straddle-counter: same DEBUG-only upstream slide as ANIMATE above (dma_queue +0xE, vblank +0x24); no length change, plain does not move.
    assert_eq!(pins::CORE.debug_len, 0x740);  // -0x2 alignment-flip: RunObjects_Frozen's `jbsr Draw_Sprite` relaxes bsr.w -> bsr.s once the 0xE quantum pad between core and sprites is gone (the target comes within short reach); DEBUG only — plain's bsr was already short  // -0xe R6: `end` re-spelled from the neighbour's head to section:core, so the pin now measures this region's own extent instead of the gap to its neighbour. Length only; the base is untouched.    // +2 addrfree: the tail align pad that absorbs debug's ≡2 (mod 4) span — placement, core.emp untouched  // +0x20 defect-batch-8
    assert_eq!(pins::DPLC.plain_base, 0x2C2A);  // +0xC ls12-refreeze: the boot slide  // -0x2c alignment-flip: upstream pad removed (the CORE slide minus dplc's own 4 B tail pad, which sits BELOW this base)  // +0x60 parcel-r1-splice  // +0x20 offscreen-ship-setreg-replay  // +0x40 offscreen-frame-top-ship  // +0x10 parcel-w: same buffers.emp plain-only slide as ANIMATE.  // -0x30 blanket-restore  // -0xE0 effects-module-split  // +0xF0 effects-p1-raster  // +0x10 item27: boot-growth slide  // +0x10 defect-batch-8  // +0x10 sst-fold  // +0x80 art-streaming-p2-task2  // +0x30 art-streaming-p2-task3  // +0x20 art-streaming-p2-task4  // +0x20 art-streaming-p2c-t8-t9  // -0x50 wave-f3-f1-f6  // +0x140 sound-pkg1  // -0x60 sound-pkg4  // +0x160 aeon-arctan  // +0x10 replay-hash-layout-proof  // +0x10 character-lens-sweep // -0x10 effects-p3-parcel-c2 (the three legacy per-field installers deleted from the effects modules; every region downstream inherits the slide) // -0x50 delete-percell-hscroll (BUFFERS plain_len 0x330->0x2E0: Parallax_Fill_PerCell/Static_Hscroll_Cell fill + DMA and the mode-key select deleted in buffers.emp; the object family sits downstream of buffers and upstream of parallax, so it inherits ONLY the buffers slide)
    assert_eq!(pins::DPLC.debug_base, 0x2F0E);  // +0xC ls12-refreeze: the boot slide  // -0x1a alignment-flip: the debug upstream slide (CORE debug -0x1e minus dplc's own 4 B tail pad)  // +0x60 parcel-r1-splice  // +0x10 offscreen-ship-setreg-replay  // +0x50 offscreen-frame-top-ship  // +0x40 blanket-restore-r2 (same flat debug slide; plain holds)  // -0x40 blanket-restore  // +0x10 effects-p2-palette (debug-only upstream align step)  // -0xF0 effects-module-split  // +0xF0 effects-p1-raster  // +0x10 item27: boot-growth slide  // +0x20 defect-batch-8  // +0xc sst-fold  // +0x80 art-streaming-p2-task2  // +0x40 art-streaming-p2-task3  // +0x10 art-streaming-p2-task4  // +0x30 art-streaming-p2c-t8-t9  // -0x50 wave-f3-f1-f6  // +0x140 sound-pkg1  // -0x60 sound-pkg4  // +0x160 aeon-arctan  // +0x10 replay-hash-layout-proof  // +0x20 character-lens-sweep // +0x10 raster-substrate-byte-moving: buffers.emp grew for the off-screen ship guard's DERIVED palette line (item 4), and the object family sits downstream of buffers. DEBUG only — plain absorbs it in existing alignment slack. // -0x60 delete-percell-hscroll (BUFFERS debug_len 0x338->0x2D8: same deletion, the debug shape gives back 0x10 more because its per-cell fill carried the DEBUG-fenced assert) // +0x32 dma-straddle-counter: same DEBUG-only upstream slide as ANIMATE above (dma_queue +0xE, vblank +0x24); no length change, plain does not move.
    assert_eq!(pins::DPLC.plain_len, 0xA4);  // -0x4 R6: `end` re-spelled from the neighbour's head to section:dplc, so the pin now measures this region's own extent instead of the gap to its neighbour. Length only; the base is untouched.     // +0xC: item-11 bcs + post-loop commit (both procs)
    assert_eq!(pins::DPLC.debug_len, 0xA4);  // -0x4 R6: `end` re-spelled from the neighbour's head to section:dplc, so the pin now measures this region's own extent instead of the gap to its neighbour. Length only; the base is untouched.   // item 6 REMOVED (soak disproved single-entry) — debug == plain  // +0x4 sst-fold
    ...
    assert_eq!(pins::DELETE_OBJECT, pins::Pin { plain: 0x2D9E, debug: 0x3082 });  // +0xC/+0xC ls12-refreeze: the boot slide, both shapes (CORE's -0xA sits after the label)  // alignment-flip: plain -0x30, debug -0x1e — rides CORE's base; the label holds its offset inside core (the debug -2 relaxation is BELOW DeleteObject)  // delete-percell-hscroll: plain -0x50, debug -0x60 - the buffers slide the whole object family takes (CORE base moved by the same amounts), the label holds its offset inside core  // +0x60 parcel-r1-splice  // offscreen-ship-setreg-replay: plain +0x20, debug +0x10  // offscreen-frame-top-ship: plain +0x40, debug +0x50  // parcel-w: plain +0x10, debug HOLDS — slides with core/dplc on the buffers.emp growth (the HScroll DMA-length key learning about pcfg_anchor_ch), and the debug shape absorbs it in existing alignment slack  // blanket-restore-r2: plain HOLDS, debug +0x40 (slides with core/dplc; the round-2 growth is DEBUG-fenced)  // blanket-restore: plain -0x30, debug -0x40 (slides with core/dplc)  // debug +0x10 effects-p2-palette (the debug-only upstream align step, same family as CORE/DPLC)  // effects-module-split: plain -0xE0, debug -0xF0  // effects-p1-raster: plain +0xF0, debug +0xF0  // +0x10 replay-hash-layout-proof  // +0x160 aeon-arctan: slides with core/dplc on the math growth  // +0x10 item27: slides with core on the boot growth  // defect-batch-8: plain +0x10, debug +0x20  // sst-fold  // +0x80 art-streaming-p2-task2  // art-streaming-p2-task3: plain +0x30, debug +0x40  // art-streaming-p2-task4: plain +0x20, debug +0x10  // art-streaming-p2c-t8-t9: plain +0x20, debug +0x30  // wave-f3-f1-f6: plain -0x50, debug -0x50  // +0x140 sound-pkg1  // -0x60 sound-pkg4  // +0x10/+0x20 character-lens-sweep  // effects-p3-parcel-c2: the three legacy per-field installers (Palette_LoadSection/Raster_InstallSection/Palette_InstallCycleSection) were deleted once total binding made them unreachable; every region downstream inherits the slide  // debug +0x10 raster-substrate-byte-moving: the same buffers.emp growth (item 4's derived ship-guard palette line) that slid the whole object family in the DEBUG shape; plain holds, as it does for ANIMATE/CORE/DPLC/RINGS in this parcel. // debug +0x32 dma-straddle-counter: rides CORE, same DEBUG-only dma_queue+vblank slide; plain holds, as it does for ANIMATE/CORE/DPLC/RINGS.
    ...
    assert_eq!(pins::OJZ_ACT_POOL.debug_len, 0x2F0C);  // -0x4 R6: `end` re-spelled from the neighbour's head to section:ojz_act_pool, so the pin now measures this region's own extent instead of the gap to its neighbour. Length only; the base is untouched.
    ...
    // effects-p3-parcel-c2 (2026-08-14): 0xDDC/0xDD0 -> 0xA8A BOTH SHAPES. Two changes
    // compound here and it is worth separating them, because only one is a size change:
    //
    //   1. configs.emp SHED its bottom half. It had grown into two unrelated libraries —
    //      parallax configs, then the game-side effects fixtures — and the effects half
    //      (OJZ_TestRaster/TestPal/ShimmerCycle/WaterRaster/GradientStream/TestGradient/
    //      TestVsram/TestRamp and their consts) moved to
    //      games/sonic4/data/effects/ojz_effects.emp. Every moved symbol was verified
    //      byte-identical against a pre-move baseline, and all four ROM CRCs were
    //      UNCHANGED across that move, so this is a relocation, not a shrink of content.
    //
    //   2. The region's `end` was repointed ObjDef_Static -> OJZ_TestRaster. Without
    //      that, the span would have kept covering BOTH sections while the port test
    //      links only parallax_configs — 0xE9C pinned against 0xA8A emitted, which the
    //      align-pad tolerance correctly refused rather than absorbing.
    //
    // The shapes now agree (0xA8A/0xA8A) where they used to differ by 0xC: the effects
    // fixtures were the only shape-sensitive content in the old span, so the remaining
    ...
    assert_eq!(pins::SCENE_REGISTRY.plain_len, 0x1486);  // +0x2BE ls12-refreeze: the registry data grew across the aeon span 483b3e12..157e59bf (scene work; not bisected per aeon commit here)  // +0x10C band-drift: 67 bands x the 4 B band_record widening (BAND_DRIFT_N 0 -> 1); records, band count and deform tables untouched  // +0x28 band-ceiling-16: 20 records x the 2 B header growth (sizeof(parallax_config) 28 -> 30 for the u16 layer_mask)  // +0x29E showcase-effects: 67 bands x 10 B, the band_record widening  // +0x44 parcel-w: ParallaxConfig_OJZ_Underwater, the anchored fixture. EXACTLY sizeof(parallax_config) 28 + 4 bands x 10 = 68 = 0x44, and shape-invariant because it is data. // +0x328 BOTH shapes: the OJZ scene/effects registry grew with the sections and presets added upstream of this freeze. A data region, so the growth is shape-invariant.
    assert_eq!(pins::SCENE_REGISTRY.debug_len, 0x1486);  // +0x2BE ls12-refreeze: same data, same size in both shapes  // +0x10C band-drift: same data, same size in both shapes  // +0x28 band-ceiling-16: same data, same size in both shapes  // +0x29E showcase-effects: same data, same size in both shapes  // +0x44 parcel-w: same record, same size in both shapes. // +0x328 BOTH shapes, same registry growth; equal lengths are the property this pair witnesses.
    ...
    // -0xCC debug. The shrink is real and shows in the REGION pins: BUFFERS len -0x50 plain
    // (0x330 -> 0x2E0) / -0x60 debug (0x338 -> 0x2D8) for the deleted per-cell HScroll fill,
    // its Static_Hscroll_Cell DMA entry and the runtime mode-key select; PARALLAX len -0x6C
    // in both shapes (0x864 -> 0x7F8 / 0x8F8 -> 0x88C) for Parallax_Fill_PerCell and the
    // fill-select twin. -0x50-0x6C = -0xBC plain and -0x60-0x6C = -0xCC debug, which is
    // exactly the slide every region from RASTER onward takes (RASTER 0x6BA4 -> 0x6AE8 /
    // 0x7DD8 -> 0x7D0C) and exactly the code delta the parcel reported (-188 B / -204 B).
    // The object family between buffers and parallax takes only the buffers half (-0x50 /
    // -0x60, the terms on ANIMATE/RINGS/CORE/DPLC above). Nothing past `org $10000` moves
    // - the sonic4 data, the sound bank and the scene registry are untouched (SCENE_REGISTRY
    // holds 0xACE/0xACE; the CAP_PER_LINE bit is now a hole in the caps word, which changes
    // no lowered record) - so the ROM-end pins absorb the whole shrink, as they have for
    // every engine-bank-only parcel. The ROM FILES still shrink (699672 -> 699340 plain,
    // 715582 -> 715252 debug): that is the deb2 symbol appendix past ASSEMBLED_LEN losing
    // the deleted names, not assembled bytes. PRESET's len 0x9A -> 0xA6 plain / 0xA6 -> 0xA2
    // debug is the bare-`end` pad sweep re-quantising against a LOAD_ART base that moved by
    // -0xBC / -0xCC, not preset.emp content (untouched by the parcel).
    // RAM: Static_Hscroll_Cell (one DMAEntry, sizeof 0xE) left engine/ram.emp, so every RAM
    // symbol after it steps -0xE in both shapes (Static_Hscroll_Line 0x8D14 -> 0x8D06 is the
    // first); the DEBUG-only PLAYER_{POS,STAT}_RING / PLAYER_RING_INDEX pages step -0x100
    // because their 256-byte alignment re-snaps after the slide, while the plain pages hold.
    // Symbol pin RETIRED, not moved: STATIC_HSCROLL_CELL, its repin.toml row hand-deleted.
    // ring-sparkle (2026-08-26): plain ASSEMBLED_LEN 0xA11C0 -> 0xA11D0, +0x10.
    // The engine call site is +8 (see the RINGS term above) and the sonic4 sparkle object
    // is 192 B in the game section — yet only 0x10 reaches the ROM tail, because the object
    // lands inside an org-anchor-absorbed run the way several parcels before it did. So
    // this delta measures the TAIL, not the parcel: do not read 0x10 as the parcel size.
    // rom-relayout (2026-08-26, aeon parcel/rom-relayout, owner ruling d-28-answered
    // option 2): plain ASSEMBLED_LEN 0xA11D0 -> 0xA5BA0, +0x49D0. The Z80 banks moved
    // from the middle of the data region ($48000/$58000) to AFTER it ($90000/$A0000)
    // by aeon's BANK PLACEMENT RULE (dac_banks = align_up(max packed_data_end + 0x4000,
    // 0x8000)); Map_Tails/Map_Knuckles came back beside the character data ahead of
    // HeightMaps (their exile retired). The tail delta is the reserve + bank pad that
    // now sits BETWEEN the data and the banks: packed data ends 0x89ED0, the banks
    // start 0x90000 (the old 0x48000 - 0x468A0 gap of 0x1760 is gone), so +0x6130 of
    // room minus the 0x1760 given back = +0x49D0 — identical in both shapes because
    // the debug shape's data end (0x8A720) rounds up to the SAME 0x90000. Every
    // sound-side pin moved exactly +0x48000 (one block), every character-data pin
    // -0x31F00 plain / -0x33100 debug (back below the banks), and the collision_data
    // pins +0x43630 (Tails + Knuckles now ahead of them). The resident Z80 blob
    // changed by exactly 4 bytes (the SND_ENGINE_TABLE_BANK / SFX_BLOB_BANK
    // immediates, $0B -> $14); nothing else in [0, ObjCodeBase) moved.
    // knux-instashield-sfx (2026-08-26): plain tail 0xA5BA0 -> 0xA5C80, +0xE0, and debug takes
    // the same +0xE0 (see its term). The shapes agreeing is the expected property: every call
    // site is inside `if SOUND_DRIVER_ENABLED == 1`, which is ON in both canonical sonic4
    // shapes and OFF in demo — which is why BOTH demo shapes stayed byte-identical.
    // The growth is four SFX blobs + their patch banks (S3K 42/4A/4C/7E) plus four table rows
    // and four call sites. It does NOT touch the fixed head layout: all four ids fall inside
    // the existing $33..$BB key range, so SFX_TABLE_LEN holds at 137, the win-tab span ensure
    // holds at 137, soundbankhead's `_sfxwin.len == $112` wall is untouched, and no
    // banked-carrier VMA moves. The set grew 11 -> 15 sounds at zero cost to the head.
    // NOT a twelfth volume envelope: sTone_17 was unmapped and is ALIASED to the shipped env
    // $0A, on the verified premise that S3K's VolEnv_16 and VolEnv_09 are byte-identical (both
    // read out of skdisasm's Z80 driver: 21 bytes each including the continuation line and the
    // 81h terminator). A real twelfth env would have grown SoundTablesZ80_Head — a hard org —
    // and slid six hand-written banked-carrier VMAs in this repo that seam1.rs itself calls
    // unchecked, which is a far larger blast radius than the alias.
    // Note the ROM grew +242 B while the assembled total moved +0xE0 (224): the remaining 18 B
    // is deb2 appendix, four new symbols' worth of name bits. Assembled length and file length
    // are different questions and this parcel separates them by a visible amount.
    // sfx-noise-tracked (2026-08-27): plain 0xA5C80 -> 0xA5C90, debug 0xA7E60 -> 0xA7E70, +0x10
    // each. B5's tone-tracked noise for the three `smpsPSGform $E7` SFX. The shapes agreeing is
    // the expected property for the same reason knux-instashield-sfx gave: every affected site is
    // inside `if SOUND_DRIVER_ENABLED == 1`, ON in both canonical sonic4 shapes and OFF in demo —
    // and BOTH demo shapes are byte-identical again (8bd3d11b / ec71a5a4).
    // WHY +0x10 AND NOT +0x12, which is what the region diff shows. SFX_BANK_BLOB really does
    // grow +0x12 (0x8DA -> 0x8EC) as the three blobs regenerate — they carry pitch notes plus the
    // `$E7` kind where the v1 approximation baked one mode byte. But regions are 0x10-aligned, so
    // the pad between the blob's end and the next region absorbed 2 of those 18 bytes:
    // 0xA3B20+0x8DA = 0xA43FA leaves 6 bytes of pad to 0xA4400, and 0xA3B20+0x8EC = 0xA440C
    // leaves 4 to 0xA4410. Downstream therefore moves +0x10, not +0x12, in both shapes (debug:
    // 0xA5570+0x8DA = 0xA5E4A -> pad 6; +0x8EC = 0xA5E5C -> pad 4). The 2 bytes are not lost or
    // absorbed by any code change; they are alignment slack that was already there.
    // The RESIDENT Z80 blob contributes ZERO here even though it SHRANK by one byte in both
    // shapes (6164 -> 6163, 6294 -> 6293): aeon pads the blob to an even length inside the
    // `Z80_Sound_Start`/`_End` brackets, so an odd length rounds back up to the same bracket size
    // and `Z80_SOUND_SIZE` holds at 0x1814 / 0x1896. That shrink is why seam1's BLOB_LEN_PLAIN /
    // BLOB_LEN_DEBUG re-pin to 0x1813 / 0x1813+0x82 — three sites, since the literal is also
    // hard-coded a second time inside `blob_lengths_are_canonical`.
    // Note the ROM grew +16 B and the assembled total moved +0x10 (16): they AGREE this time,
    // unlike knux-instashield-sfx where the deb2 appendix added a visible 18 B on top. This
    // parcel declares no new symbols — it deletes `Psg_Noise`'s legacy SFX arm and moves a route
    // test into `Psg_EmitDivisor` — so the name trie does not grow.
    assert_eq!(pins::ASSEMBLED_LEN, 0xBDC92);  // +0x10 ls12-refreeze: EndOfRom 0xBDC82 -> 0xBDC92 across the aeon span 483b3e12..157e59bf (the resident blob's +0xC and the rest of the step's growth, netted through the tail's pad; not bisected per aeon commit here)  // -0xe alignment-flip: the ROM tail packs flush too — Sfx_33 gives back 8 of inferred 16-quantum pad, the game sections behind it 4, replay_fixture 2 (all declared WORD/8, no 16-quantum); the engine bank's -0x98 is org-anchor absorbed at $10000 as ever  // HOLDS band-first-consumer (the band PROGRAM's emission is DEBUG-gated after a dormant-scaffold ruling, so the release shape emits nothing; s4.bin's CRC still moved, but a byte-region compare puts all 2241 differing bytes in the header checksum plus deb2 appendix ABOVE the last emitted symbol 0xA5C90 -- zero EMITTED release bytes, which is why this total holds while the CRC does not)  // HOLDS debug-rings-gate (release code, NOT shape-gated — the ~12 bytes of gate are absorbed by section padding inside ojz_scroll_test, whose plain_len is unchanged at 0x570; the release FILE still grew +30 for the new .skip_rings deb2 record, which is what proves the code is in the release shape rather than DEBUG-fenced)  // +0x10 sfx-noise-tracked (three $E7 SFX blobs regenerate +0x12; 0x10-region alignment gives 2 back)  // +0xE0 knux-instashield-sfx (four S3K SFX blobs + rows + call sites; head layout untouched, ids inside the existing key range)  // +0x49D0 rom-relayout  // HOLDS delete-percell-hscroll (engine-bank -0xBC absorbed by `org $10000`; see the narrative above)  // +0x10 ring-sparkle (tail only; the object is org-anchor absorbed)  // offscreen-ship-setreg-replay  // +0x10 offscreen-frame-top-ship: the ship's enqueue block + Build_DMA_Entry's hoist out of BuildStaticDMA + Raster_BuildShipEntry, minus what PatchAll gave back when it stopped deriving its own screen line.  // -0x20 effects-p3-parcel-p-b (the single-channel water slot deleted: Raster_InstallWater/InstallPatchedWorldY/PatchWaterLine/PatchWaterWorldY out, Raster_PatchAll/InstallPatched/Effects_SetWorldY in — a net shrink even after the new routines, and the -4 RAM of the deleted water pair is org-anchor absorbed) // -0x10 effects-p3-parcel-c2 (the legacy per-field installers deleted; the relocation itself was byte-neutral) // -0x10 blanket-restore: only the configs data shrink reaches the tail; the engine-bank -0x30 is org-anchor absorbed // +0x30 effects-p2-palette: the +0x560 engine growth is org-anchor absorbed (as it has been for several chains) and only this reaches the ROM tail // +0x20F60 tails-data (Map_Tails exiled to the ROM tail) // +8 character-dispatch-c1 (0x50 of player growth, repacked past the sound bank) // +0xF0 slide-fixture; patchrun/rerecord/sound-pkg1 absorbed  // +0x30 player-polish-trio  // +0x30 sound-pkg3 (v2: +0x10 mod-8 base pads after the fold-divergence fix)  // +0xC0 sfx-flight  // +0x10 dust-data (ojz_scroll_test's puff DMA; the 3 KB data insertion is dac-anchor-absorbed)  // +0x226D0 knuckles-def (Map_Knuckles takes the same ROM-tail exile) // +0x18000 rom-relayout-more-room: the DAC and sound banks move to 0xA8000/0xB8000, and the plain shape's whole tail rides them. EXACTLY the anchor delta and nothing else — the plain shape's content growth all lands upstream of the banks, in slack.
    ...
    // ring-sparkle (2026-08-26): debug tail 0xA3290 -> 0xA32A0, +0x10 — the same delta as
    // plain and for the same reason (see the plain term above): the +8 engine call site plus
    // a game-section object that is org-anchor absorbed. The two shapes agreeing is the
    // expected property, since neither the hook nor the sparkle object is shape-gated.
    // rom-relayout (2026-08-26): debug tail 0xA32A0 -> 0xA7C70, +0x49D0 — the same delta
    // as plain and for the reason given in the plain term (both data ends round to the
    // same 0x90000 bank boundary under the rule).
    // effects-lab-hotkey (2026-08-26): debug tail 0xA7C70 -> 0xA7D80, +0x110, and the PLAIN
    // total deliberately HOLDS at 0xA5BA0. The shapes diverging is the expected property
    // here, not an anomaly: Debug_SceneCycleHotkey's whole body including its rts is inside
    // `if DEBUG == 1`, so the release shape emits no bytes for it — the same idiom
    // Debug_CharacterHotkey and Debug_Warp_Consume use, and the same shape asymmetry
    // tails-character and debug-warp-mailbox recorded above.
    // Why zero release bytes did not move the release deb2 appendix either (the trap the
    // audit-off-fill-path term above was bitten by): a zero-byte proc still mints a release
    // LABEL, and a label at a NEW address lands in the appendix and moves the ROM-end word
    // and checksum. This one parks on $A44EE, where Debug_CharacterHotkey's record already
    // sits, and convsym keeps one record per address — verified against the shipped release
    // appendix rather than assumed: 2052 records, zero addresses carrying more than one.
    // The precondition is therefore NOT "emits no code" but "no NEW record, and the surviving
    // NAME at that address is unchanged" (appendix size depends on Huffman-coded name bits).
    // s4.bin holding byte-identical at f81c6811 is what witnesses the name did not change.
    // knux-instashield-sfx (2026-08-26): debug tail 0xA7D80 -> 0xA7E60, +0xE0 — identical to
    // plain, and identical for the reason given there: the SFX sites are gated on
    // SOUND_DRIVER_ENABLED, not on DEBUG, so neither shape carries anything the other lacks.
    // Contrast the effects-lab term directly above, where the shapes deliberately diverge
    // because that proc is DEBUG-gated. Two adjacent parcels, opposite shape behaviour, both
    // correct — the gate each one hangs on is what decides it.
    // debug-rings-gate (2026-08-28): debug tail 0xA7E70 -> 0xA7E80, +0x10, and the PLAIN
    // total HOLDS at 0xA5C90. The shapes diverge here for the OPPOSITE reason to the two
    // terms above: effects-lab-hotkey and tails-character diverge because their bodies are
    // inside `if DEBUG == 1`, so the release shape emits nothing. This gate is ORDINARY
    // RELEASE CODE — debug-fly is a runtime cheat (CHEAT_DEBUG_FLY in Cheat_Flags) that
    // ships in every shape, and a build-time gate would have left the bug live in exactly
    // the shape where a cheat code can arm the bit. Both shapes emit the three instructions;
    // only the ABSORPTION differs. ojz_scroll_test's plain_len is unchanged at 0x570 while
    // its debug_len takes 0x97C -> 0x98C, i.e. the release section had padding to spend and
    // the debug section did not.
    // The discriminator that separates "absorbed" from "DEBUG-fenced", since those look
    // identical in this ledger: the release FILE grew +30 (718999 -> 719029) for the new
    // .skip_rings record in the deb2 appendix. A DEBUG-gated proc mints no release label at
    // all, so a release appendix that moved is positive evidence the code is present in the
    // release shape and merely absorbed — the inverse of the effects-lab-hotkey term, which
    // had to argue its release appendix did NOT move.
    // band-first-consumer (2026-08-28): debug tail 0xA7E80 -> 0xA7EC0, +0x40, and the PLAIN
    // total HOLDS at 0xA5C90. This is the effects-lab-hotkey shape, NOT the debug-rings-gate
    // shape directly above it, and the distinction is the parcel's whole story: the rings gate
    // was release code that the plain section absorbed, so plain held by ABSORPTION. Here the
    // band program's EMISSION is DEBUG-gated, so plain holds because it emits nothing at all.
    // Two identical-looking HOLDS with different causes; the discriminator is that the rings
    // gate's release FILE grew (+30, a deb2 record for a label the release shape really minted)
    // while this parcel's release file is the same LENGTH with a different CRC -- 2241 bytes
    // differing, every one of them the header checksum or appendix above 0xA5C90.
    // v1 of this parcel emitted the program unconditionally and shipped it into the release ROM
    // behind a zero-byte installer -- unreachable data in the artifact that ships. Gated after a
    // dormant-scaffold ruling; the +0x40 is what the debug shape pays for carrying both.
    // editor-raster-preset (2026-08-29): debug tail 0xA7EC0 -> 0xA7F40, +0x80, and the PLAIN
    // total HOLDS at 0xA5C90. This term is the DELIBERATE INVERSE of band-first-consumer
    // directly above it, and the inversion is the whole point rather than a regression.
    // That parcel's v1 emitted its raster program unconditionally, shipped unreachable data
    // into the release artifact, and was GATED after a dormant-scaffold ruling. This parcel
    // emits unconditionally ON PURPOSE, under an explicit ruling (aeon e99a2ca7): 16.2a
    // governs APPARATUS, and OJZ_BandDemo is apparatus — it has no consumer outside DEBUG and
    // never will. An EDITOR-AUTHORED preset is CONTENT, and its install path is DEBUG-only
    // only because the section-binding half has not landed. Gating content on a build shape
    // would make the GENERATOR's output shape depend on DEBUG, so a preset later bound into a
    // section would emit a zero-length program in release alone — a silent shape divergence
    // no gate in either tree could catch, which is strictly worse than dormant bytes.
    // WHERE THE +0x80 IS, measured per region rather than argued: ojz_scroll_test is the
    // WHOLE of it — plain_len HOLDS at 0x570 while debug_len takes 0x9CC -> 0xA4C, exactly
    // +0x80, which is the DEBUG_ASSEMBLED_LEN delta with nothing left over. That is
    // Debug_BandDemoHotkey becoming a cycle: the .raster_table, the cursor read, the range
    // assert and the advance, all inside `if DEBUG == 1`, so plain emits none of it.
    // WHERE THE PAYLOAD IS, and why it is NOT in that number: the editor-authored program is
    // 78 B (0x4E) and is emitted in BOTH shapes, upstream of OJZ_EFFECTS — whose base moves
    // 0x1323C -> 0x1328A, +0x4E exactly, in plain as well as debug (its own length takes +2
    // in both). The bank anchor absorbs those 0x4E in both shapes, which is why plain HOLDS
    // despite genuinely carrying the program. So this parcel has BOTH mechanisms at once: a
    // shape-divergent DEBUG body (the effects-lab-hotkey shape) and an absorbed unconditional
    // payload (the debug-rings-gate shape), and reading either alone gets the story wrong.
    // The discriminator that separates "absorbed" from "not present", since a held
    // ASSEMBLED_LEN looks identical either way: the release FILE grew +30 (719205 -> 719235)
    // while ASSEMBLED_LEN held, which is the deb2 appendix taking records for labels the
    // release shape really minted. That is positive evidence the program IS in s4.bin — the
    // exact inverse of band-first-consumer, which had to argue its release file was the same
    // LENGTH with a different CRC because its emission was gated away.
    assert_eq!(pins::DEBUG_ASSEMBLED_LEN, 0xC14F4);  // +0xF96 ls12-refreeze: the debug EndOfRom 0xC055E -> 0xC14F4 across the aeon span 483b3e12..157e59bf (not bisected per aeon commit here)  // +0x1E0 chain 202 (aeon cb0e5eb1, "the debug build says which scene is on screen") and +0xE4 chain 206 (aeon 36285940, the live-object spawn mailbox): the corpus-pin-advance span's ONLY two moving points, each MEASURED AT ITS OWN LANDING by the aeon lane (13 canonical DEBUG builds across 8876459e..4f5ad5a1, EndOfRom read from each listing) rather than back-derived from the total this assert exists to check. Both are DEBUG-ONLY content and the attribution was verified here rather than taken: every code hunk in both merges lands in games/sonic4/test/ojz_scroll_test.emp (292 and 262 insertions; 4 and 11 added DEBUG guards), which is why ASSEMBLED_LEN above HOLDS at 0xA5C82 across all thirteen points. The other ten points are EXPLICIT measured HOLDS, not gaps inferred from their neighbours. Two self-checks, both stated before the series arrived: the endpoints reproduce 0xA7F38 and 0xA81FC, and 0x1E0 + 0xE4 = 0x2C4 exactly.  // +4 item5-cycles-variants (chain 198, aeon 73b07a4f): MEASURED, not inferred from the total. `Debug_BandDemoHotkey`'s `.raster_table` gains one dc.l row for the new `EditorRaster_OJZ_Act1_ojz_sec3_shimmer` — its span is 8 B at aeon 8dd28114 and 12 B at 73b07a4f (table start 0xA6886 to the next symbol `OJZ_SectionMarkerColors`, which is itself the FIRST symbol carrying the +4), so the table's growth and the tail's delta are the same 4 rather than two facts that happen to agree. Plain HOLDS at 0xA5C82 for the strongest available reason: `Debug_BandDemoHotkey$raster_table` appears ZERO times in s4.lst at both revisions, so the table does not exist in the release shape. NOT because the hotkey is debug-only — the enclosing proc `Debug_BandDemoHotkey` IS in the release shape, at 0xA45DC — it is the `$raster_table` LOCAL that the release build does not emit, and that distinction is the one a later reader will get wrong because the other sentence is easier to write. The zero is a real absence rather than a failed query, and the control is a POSITIVE read rather than a trusted empty one: `grep raster_table s4.lst` is NOT empty — it returns the unrelated equate `Sec_sec_raster_table = $00000018` — so the instrument demonstrably finds that string in that file and still finds no table label. FILE-DELTA DECOMPOSITION, since the tempting one is wrong and entry 198's own frozen prose carries it: release +93 = 0 assembled + 93 appendix, debug +100 = 4 assembled + 96 appendix. The parcel's 62 content bytes ship in BOTH shapes and are org-anchor absorbed, so they are not a term in either file delta; counting them at the tail double-counts them, and `anchor_end = 0xa5c82` holding is that story's own refutation. The appendix column is file minus EndOfRom, which is legitimate here because the deb2 magic sits at EXACTLY EndOfRom — `deb20402` read at that offset in all four ROMs, verified here. The item-5 PAYLOAD is a separate +0x3E on 230 symbols (`OJZ_TestRaster` through `Art_Sonic`) and reaches neither total — it is absorbed whole at the `dac_banks` org anchor, the debug delta returning to +0 at `Dac_Temp_Blip` 0x90000. Two independent absorptions in one parcel; reading the tail's 4 as the payload's residue gets it backwards.  // -0xc alignment-flip: Sfx_33 -8 and the game sections -4 as in plain, PLUS the debug shape's own abs.w ceiling crossing — Raster_Install slides 0x8042 -> 0x7FDC, so Debug_BandDemoHotkey's two `jbra Raster_Install` re-encode jmp abs.l -> abs.w (-4), of which replay_fixture's declared-align pad gives 4 back; the debug engine bank's -0x80 is org-anchor absorbed  // +0x80 editor-raster-preset (DEBUG-gated CYCLE machinery in ojz_scroll_test, +0x80 and the whole delta; the 0x4E editor-authored payload is emitted in BOTH shapes and bank-absorbed, so plain HOLDS while genuinely carrying it -- see the paragraph above, and note this is the deliberate INVERSE of the gated parcel below)  // +0x40 band-first-consumer (DEBUG-gated EMISSION: three-band raster program + its START+UP installer; plain emits nothing -- see the paragraph above for why this HOLDS for a different reason than the rings gate below)  // +0x10 debug-rings-gate (release code, absorbed in plain and not in debug — see the paragraph above; NOT a DEBUG gate)  // +0x10 sfx-noise-tracked (same delta as plain; SOUND_DRIVER_ENABLED-gated, not DEBUG-gated)  // +0xE0 knux-instashield-sfx (same delta as plain; SOUND_DRIVER_ENABLED-gated, not DEBUG-gated)  // +0x110 effects-lab-hotkey (DEBUG-only: the proc + its 20-entry scene table + the frame-top call in GameState_OJZScroll_Update; plain HOLDS — see the paragraph above for why the release appendix did not move either)  // +0x49D0 rom-relayout  // HOLDS delete-percell-hscroll (engine-bank -0xCC absorbed the same way)  // +0x10 ring-sparkle (tail only)  // +0x30 parcel-w (the world-anchored overlay: parallax.emp +0x15E for Step 4b and raster.emp +0x34 for Raster_GetChannelBand, against preset.emp -0x2 for the dropped anchor-pointer lea and the debug shape re-absorbing the rest in alignment slack; ParallaxConfig_OJZ_Underwater is data and shape-invariant at +0x44) // +0x20 effects-p3-parcel-p-b (the shapes move in OPPOSITE directions here and that is the DEBUG-only content, not an anomaly: plain takes -0x20 from the deleted single-channel water slot, while debug pays that back and more for ojz_scroll_test's DEBUG-gated Effects_SetWorldY hotkey — the same shape asymmetry tails-character recorded) // -0x10 effects-p3-parcel-c2 (legacy per-field installers deleted; the configs relocation was byte-neutral) // -0x20 blanket-restore: the configs -0x10 plus boot's -0x10, which the debug shape does not re-absorb // +0x20 effects-p2-palette: same org-anchor absorption; the debug tail takes 0x20 where plain takes 0x30 // +4 tails-appendage (the DEBUG-only jmp abs.l tail call) // -0x10 tails-flight (bank absorbs flight; ojz harness shrank) // +0xC4 tails-character (DEBUG-only hotkey; plain unmoved) // +0x20F60 tails-data (same exile) // +0x10 character-dispatch-c1 (0xB0 of player growth, repacked past the sound bank) // +6 cheat-flag arm write; +0x34 objtest-gate moves; +0xF0 slide-fixture; +8 replay-rerecord; sound-pkg1 absorbed  // +0x30 player-polish-trio  // +0x30 sound-pkg3 (v2: +0x10 mod-8 base pads after the fold-divergence fix)  // +0xC0 sfx-flight  // +0x18 dust-data (ojz_scroll_test's puff DMA; the 3 KB data insertion is dac-anchor-absorbed)  // +0x226D0 knuckles-def (same exile, same delta — the data is not DEBUG-fenced)  // +0x10 knuckles-c4 (glide family; plain repacked past the sound bank, debug keeps the residual)  // +0xF0 debug-warp-mailbox (DEBUG-only: Warp_Req_* cells are @shape_divergent game RAM, the frame-top consumer + PageCache_ResetRefcounts + reseed sequence live only in the DEBUG shape; releases byte-identical cdabf8a3/f7806241 by the deb2 label-parking documented in the parcel — plain total deliberately unmoved)  // +0x20 audit-off-fill-path (F5, DEBUG-only: Tile_Cache_Fill -0x14 losing the interval gate, GameState_OJZScroll_Update +0x16 gaining the sibling call + gate, net +2 rounded to alignment; releases byte-identical — the engine-proc spelling was REVERTED in-parcel precisely because its zero-byte label moved demo release deb2)  // +0xB0 debug-boot-override (DEBUG-only: the Boot_At_* consumption block in GameState_OJZScroll_Init + the shared clamp/centre comptime templates instantiated at both mailbox sites; file grew only +0x5C because the deb2 appendix re-balanced — templates declare no symbols, the name trie shrank. trailing-fire and F1 deliberately absent from this ledger: both were fill/appendix-absorbed and the assembled total never moved) // +0x18330 rom-relayout-more-room: +0x18000 from the same bank move, plus 0x330 of DEBUG-only growth in games/sonic4/test/ojz_scroll_test.emp, which sits between GameState_OJZScroll_Init and Replay_OJZ_Fixture and so lands after the banks. The plain shape has no such region and takes the anchor delta alone.  // +0x32 relayout-at-aeon-master (aeon 483b3e12): DERIVED from the two listings, not from the total. Walking s4.debug.lst's 2,963 common symbols in address order, the whole pre-bank story is absorbed at `Dac_Temp_Blip` 0xA8000 (delta returns to +0 there from a mixture of +0x8 player_common, +0x1A generated editor rasters, +0x30 OJZ_Reels_Fill and the Sec-record shrink's -0xF0/-0x1BA), and exactly TWO points move after it, both inside games/sonic4/test/ojz_scroll_test.emp's DEBUG-only region: +0x12 at `Debug_PresetReadout_Show$verdict_live` (0xBEDA4 -> 0xBEDB6) and a further +0x20 by `OJZ_SectionMarkerColors` (0xBEE8A -> 0xBEEBC), which then carries unchanged to EndOfRom 0xC052C -> 0xC055E. ASSEMBLED_LEN HOLDS at 0xBDC82 for the reason the plain walk shows directly: its only transitions are +0x8 at `Player_LevelBound`, -0x118 at `OJZ_Sec0_Blocks` and -0x1E2 at `OJZ_Sec5_LocalMap`, all of them before the anchor and all absorbed by it, so plain's file grew by 8 bytes of deb2 appendix while its assembled length did not move at all.
...
fn secondary_pin_classes_match_the_hand_typed_baseline() {
    ...
    assert_eq!(pins::SOUND_API.plain_base, 0x7A9E);  // +0x20 offscreen-ship-setreg-replay  // +0xC0 offscreen-frame-top-ship  // +0x1B0 parcel-w: sound sits downstream of BOTH growths, so it absorbs the object-family 0x10 plus parallax.emp +0x15E (the Step-4b overlay) plus raster.emp +0x34 (Raster_GetChannelBand) less preset.emp -0x2 (the dropped anchor-pointer lea).  // +0x560 effects-p2-palette (flat downstream slide: game_loop 0x6 + raster 0xDA + palette 0x480)  // +0x10 effects-module-split  // +0x100 effects-p1-raster  // +0x50 item28-transpose: section +0x20 (per-column RedrawPlanes blit) + bg +0x30 (column blit + move.l tile guard) slide everything downstream  // +0x30 defect-batch-8  // +0x10 sst-fold  // -0x20 ltr-mul: tile_cache/section multiply shrink slides everything downstream  // +0x100 art-streaming-p2-task3 (folds a prior missed round): +0x80 the task-2 zx0_resume slide the baseline never took, plus +0x80 this task (vblank +0x30 + page_in +0x5A, aligned)  // +0x180 art-streaming-p2-task4: vblank +0x20 + page_in plain len 0x5A->0x1A6 (the FIFO/procs replacing the debug-only scaffold)  // +0x520 wave-f3-f1-f6  // +0x20 wave-panel-closing  // +0x1D0 sound-pkg1  // +0x20 player-polish-trio  // -0x60 sound-pkg4  // +0x20 t1-draw-unroll  // +0x170 replay-hash-layout-proof  // +0x40 character-lens-sweep  // +0x110 STALE-CATCHUP: this row was already 0x110 behind the COMMITTED pins.rs before the raster-substrate parcel (baseline 0x796E vs pins 0x7A7E), i.e. this test was RED on master and the earlier assertions were passing, so the failure surfaced here. An earlier parcel moved sound and did not re-baseline. Recorded as its own term rather than folded into the line below, so the next reader is not told this parcel moved sound by 0x130.  // +0x20 raster-substrate-byte-moving: sound sits downstream of palette+buffers+raster growth.
    assert_eq!(pins::SOUND_API.debug_base, 0xA330);  // +0x10 offscreen-ship-setreg-replay  // +0xD0 offscreen-frame-top-ship  // +0x390 parcel-w: the debug shape takes a larger step than plain, the usual debug-only alignment behaviour for this row. The prior chain entry warns against assuming SOUND_API is always the odd row; it is re-derived here, not pattern-matched.  // +0x570 effects-p2-palette (the same 0x560 + the 0x10 debug-only upstream step)  // -0x10 effects-module-split  // +0x100 effects-p1-raster  // +0x50 item28-transpose: same slide, both shapes  // -0x20 defect-batch-8  // +0x10 sst-fold  // -0x20 ltr-mul  // +0x2F0 art-streaming-p2-task3 (folds a prior missed round): +0x150 the task-2 slide the baseline never took, plus +0x1A0 this task (boot +0x10 + vblank +0x30 + page_in +0x166)  // +0x10 art-streaming-p2-task4: vblank +0x10 + load_art/page_in debug net (load_art len -0x40, page_in base -0x30 + len +0x40)  // +0xd90 wave-f3-f1-f6  // +0x90 wave-panel-closing  // +0x2A0 sound-pkg1  // +0x10 player-polish-trio  // -0x60 sound-pkg4  // +0x20 t1-draw-unroll  // +0x180 replay-hash-layout-proof  // +0x50 character-lens-sweep  // +0x110 STALE-CATCHUP: same pre-existing drift as the plain row (baseline 0xA1F0 vs committed pins 0xA300).  // +0x30 raster-substrate-byte-moving: the debug shape takes the larger step, as this row usually does.
