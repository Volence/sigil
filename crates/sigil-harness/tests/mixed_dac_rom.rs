...
fn emp_bank_map_with_mt_hblank(debug: bool) -> String {
    ...
    let hblank_base = if debug { "0x2380" } else { "0x22F2" };
    format!(
        "fill = 0x00\n\
         \n\
         [[region]]\n\
...
fn emp_bank_map_with_mt_hblank_tranche2(debug: bool) -> String {
    ...
    let hblank_base = if debug { "0x2380" } else { "0x22F2" };
    let controllers_base = if debug { "0x2392" } else { "0x2304" };
    let math_base = if debug { "0x2672" } else { "0x24E0" };
    format!(
        "fill = 0x00\n\
         \n\
         [[region]]\n\
...
fn emp_bank_map_tranche5(debug: bool) -> String {
    let game_loop_base = if debug { "0x2408" } else { "0x237A" };
    let sound_api_base =
...
fn mixed_tranche5_rom_matches_assembled_reference() {
    ...
    let sdsr = (pins::SOUND_DRAIN_SFX_RING.plain as i64 - 0x2380) as u16;
...
fn mixed_tranche5_debug_rom_matches_assembled_reference() {
    ...
    let sdsr = (pins::SOUND_DRAIN_SFX_RING.debug as i64 - 0x240E) as u16;
    // `movea.l (Game_State).w,a0` — abs.w RAM target, pin-spliced (tranche-12).
