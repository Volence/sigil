...
fn mixed_tranche3_rom_matches_assembled_reference() {
    ...
    // ($4336 - $4CBE = -$988 = $F678) — the cross-seam pc-relative TAIL
    // CALL. The target Tile_Cache_GetCollision stays at $4336 (it precedes the
    // insertion points); Collision_GetType's bra moved +$10 when unified-prefetch
    // H5 grew BlockStage_PtrTable (12→16 slots), so the backward disp grew -$10
    // ($F688 → $F678 plain, $F5C8 → $F5B8 debug).
    ...
    assert_eq!(
        ...
        &[
            ...
            0xB2, 0x78, ((clc + 4) >> 8) as u8, (clc + 4) as u8, 0x6D, 0x0A, 0xB2, 0x78, ((clc + 6) >> 8) as u8, (clc + 6) as u8, 0x6E, 0x04, 0x60, 0x00, 0xF6, 0x78,
            0x70, 0x00, 0x4E, 0x75,
        ][..],
...
fn mixed_tranche3_debug_rom_matches_assembled_reference() {
    ...
    assert_eq!(
        ...
        &[
            0xE6, 0x48, 0xB0, 0x78, (clc >> 8) as u8, clc as u8, 0x6D, 0x18, 0xB0, 0x78, ((clc + 2) >> 8) as u8, (clc + 2) as u8, 0x6E, 0x12, 0xE6, 0x49,
            0xB2, 0x78, ((clc + 4) >> 8) as u8, (clc + 4) as u8, 0x6D, 0x0A, 0xB2, 0x78, ((clc + 6) >> 8) as u8, (clc + 6) as u8, 0x6E, 0x04, 0x60, 0x00, 0xF5, 0xB8,
            0x70, 0x00, 0x4E, 0x75,
        ][..],
