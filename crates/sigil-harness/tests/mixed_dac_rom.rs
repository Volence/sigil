...
fn emp_bank_map_with_mt_hblank(debug: bool) -> String {
    ...
    let hblank_base = if debug { format!("{:#x}", pins::HBLANK.debug_base) } else { format!("{:#x}", pins::HBLANK.plain_base) };
    format!(
        "fill = 0x00\n\
         \n\
         [[region]]\n\
...
fn emp_bank_map_with_mt_hblank_tranche2(debug: bool) -> String {
    ...
    let hblank_base = if debug { format!("{:#x}", pins::HBLANK.debug_base) } else { format!("{:#x}", pins::HBLANK.plain_base) };
    let controllers_base = if debug { format!("{:#x}", pins::CONTROLLERS.debug_base) } else { format!("{:#x}", pins::CONTROLLERS.plain_base) };
    let math_base = if debug { format!("{:#x}", pins::MATH.debug_base) } else { format!("{:#x}", pins::MATH.plain_base) };
    format!(
        "fill = 0x00\n\
         \n\
         [[region]]\n\
...
fn emp_bank_map_tranche3(debug: bool) -> String {
    let vdp_init_base = if debug { format!("{:#x}", pins::VDP_INIT.debug_base) } else { format!("{:#x}", pins::VDP_INIT.plain_base) };
...
fn emp_bank_map_tranche5(debug: bool) -> String {
    let game_loop_base = if debug { format!("{:#x}", pins::GAME_LOOP.debug_base) } else { format!("{:#x}", pins::GAME_LOOP.plain_base) };
    let sound_api_base =
...
fn mixed_tranche3_rom_matches_assembled_reference() {
    ...
    // Window + pc-rel disp derive from the pins (boot sits upstream of every
    // engine region — hardcoded engine addresses here go stale on any boot
    // byte-change; the row-1257 migration).
    let vb = pins::VDP_INIT.plain_base as usize;
    let disp = (pins::BOOT_DATA_VDP_REGS.plain as i32 - (pins::VDP_INIT.plain_base as i32 + 2)) as i16 as u16;
    assert_eq!(
        &rom[vb..vb + pins::VDP_INIT.plain_len],
        &[
            0x41, 0xFA, (disp >> 8) as u8, disp as u8, 0x43, 0xF8, (v >> 8) as u8, v as u8, 0x70, 0x12, 0x12, 0xD8, 0x51, 0xC8, 0xFF, 0xFC,
            0x42, 0xB8, ((v + 0x14) >> 8) as u8, (v + 0x14) as u8, 0x4E, 0x75, 0x22, 0x38, ((v + 0x14) >> 8) as u8, (v + 0x14) as u8, 0x67, 0x2C, 0x41, 0xF8, (v >> 8) as u8, v as u8,
            0x43, 0xF9, 0x00, 0xC0, 0x00, 0x04, 0x30, 0x3C, 0x80, 0x00, 0x74, 0x00, 0x76, 0x12, 0xE2, 0x89,
            0x64, 0x06, 0x10, 0x30, 0x20, 0x00, 0x32, 0x80, 0x06, 0x40, 0x01, 0x00, 0x52, 0x42, 0x4A, 0x81, 0x57, 0xCB,
            0xFF, 0xEC, 0x42, 0xB8, ((v + 0x14) >> 8) as u8, (v + 0x14) as u8, 0x4E, 0x75,
        ][..],
    ...
    // ($4336 - $4E40 = -$B0A = $F4F6) — the cross-seam pc-relative TAIL
    // CALL. The target Tile_Cache_GetCollision stays put (it precedes the
    // insertion points); Collision_GetType's bra moved when unified-prefetch grew
    // tile_cache, then again in pass-2 (1.1a FillRow nametable segments +$88,
    // 1.1b FillRow collision segments +$56, 1.3 CopyBlockColumn wrap-split +$42,
    // 8b prefetch scan memoize +$90, 8b NT move.l rider +$1C — all tile_cache-internal,
    // between the bra target and the bra), so the backward disp grew accordingly
    // (now $F2FE plain, $F23E debug).
    ...
    assert_eq!(
        ...
        &[
            ...
            0xB2, 0x78, ((clc + 4) >> 8) as u8, (clc + 4) as u8, 0x6D, 0x0A, 0xB2, 0x78, ((clc + 6) >> 8) as u8, (clc + 6) as u8, 0x6E, 0x04, 0x60, 0x00, 0xF2, 0xFE,
            0x70, 0x00, 0x4E, 0x75,
        ][..],
...
fn mixed_tranche3_debug_rom_matches_assembled_reference() {
    ...
    // Pin-derived window + pc-rel disp (see the plain arm's note).
    let vb = pins::VDP_INIT.debug_base as usize;
    let disp = (pins::BOOT_DATA_VDP_REGS.debug as i32 - (pins::VDP_INIT.debug_base as i32 + 2)) as i16 as u16;
    assert_eq!(
        &rom[vb..vb + pins::VDP_INIT.debug_len],
        &[
            0x41, 0xFA, (disp >> 8) as u8, disp as u8, 0x43, 0xF8, (v >> 8) as u8, v as u8, 0x70, 0x12, 0x12, 0xD8, 0x51, 0xC8, 0xFF, 0xFC,
            0x42, 0xB8, ((v + 0x14) >> 8) as u8, (v + 0x14) as u8, 0x4E, 0x75, 0x22, 0x38, ((v + 0x14) >> 8) as u8, (v + 0x14) as u8, 0x67, 0x2C, 0x41, 0xF8, (v >> 8) as u8, v as u8,
            0x43, 0xF9, 0x00, 0xC0, 0x00, 0x04, 0x30, 0x3C, 0x80, 0x00, 0x74, 0x00, 0x76, 0x12, 0xE2, 0x89,
    ...
    assert_eq!(
        ...
        &[
            0xE6, 0x48, 0xB0, 0x78, (clc >> 8) as u8, clc as u8, 0x6D, 0x18, 0xB0, 0x78, ((clc + 2) >> 8) as u8, (clc + 2) as u8, 0x6E, 0x12, 0xE6, 0x49,
            0xB2, 0x78, ((clc + 4) >> 8) as u8, (clc + 4) as u8, 0x6D, 0x0A, 0xB2, 0x78, ((clc + 6) >> 8) as u8, (clc + 6) as u8, 0x6E, 0x04, 0x60, 0x00, 0xF2, 0x3E,
            0x70, 0x00, 0x4E, 0x75,
        ][..],
...
// ===========================================================================
// Tranche 20 — the dma_queue + load_art mixed arm. INDEPENDENT of the
// sound/hblank ladder (the gates compose freely, R6): the AS side assembles
// the real full ROM with ONLY `SIGIL_EMP_DMA_QUEUE` + `SIGIL_EMP_LOAD_ART`
// on, the .emp side supplies exactly those two windows, and the union must
// reproduce the SHIPPED per-shape ROM (modulo the derived convsym bytes).
// ===========================================================================

/// Lower one tranche-20 module with its ambient twins, place into a
/// two-region map (`text` carrier + its own pinned window), return sections
/// + link asserts.
fn t20_lower_and_place(
    aeon: &Path,
    emp_rel: &str,
    ...
    assert!(
        pdiags.iter().all(|d| d.level != sigil_span::Level::Error),
        "{emp_rel} place errors: {pdiags:?}"
    );
    (sections, module.link_asserts)
}

/// `engine.vdp`'s items (the typed VDP command vocabulary + vdp_comm_reg —
/// dma_queue.emp is the ladder's first vdp_comm_reg consumer).
fn vdp_ambient_items(engine_dir: &Path) -> Vec<sigil_frontend_emp::ast::Item> {
    let src = std::fs::read_to_string(engine_dir.join("vdp.emp"))
        .unwrap_or_else(|e| panic!("cannot read vdp.emp: {e}"));
    let (file, vdiags) = parse_str(&src);
    assert!(
...
fn build_mixed_tranche20_rom(aeon: &Path, debug: bool) -> Vec<u8> {
    let as_module =
        assemble_mixed_tranche20_as_side(aeon, debug).unwrap_or_else(|e| panic!("{e}"));

    let engine_dir = aeon.join("engine");
    let system_dir = engine_dir.join("system");

    let dq = if debug { (pins::DMA_QUEUE.debug_base, pins::DMA_QUEUE.debug_len) } else { (pins::DMA_QUEUE.plain_base, pins::DMA_QUEUE.plain_len) };
    let (dq_sections, dq_asserts) = t20_lower_and_place(
        aeon,
        "engine/system/dma_queue.emp",
        vec![
    ...
    let la = if debug { (pins::LOAD_ART.debug_base, pins::LOAD_ART.debug_len) } else { (pins::LOAD_ART.plain_base, pins::LOAD_ART.plain_len) };
    let (la_sections, la_asserts) = t20_lower_and_place(
        aeon,
        "engine/level/load_art.emp",
        vec![
            structs_ambient_items(&engine_dir),
            constants_ambient_items(&system_dir),
        ],
    ...
    let mut sections = as_module.sections;
    sections.extend(dq_sections);
    sections.extend(la_sections);

    let resolved = sigil_link::resolve_layout(&sections, &SymbolTable::new(), true)
        .unwrap_or_else(|d| panic!("tranche20 mixed resolve_layout failed: {d:?}"));
    let linked = sigil_link::link(&resolved, &SymbolTable::new())
        .unwrap_or_else(|d| panic!("tranche20 mixed link failed: {d:?}"));

    // The prepended twins' drift walls must genuinely run against the REAL
    // AS-side symbols and pass.
    let mut all = dq_asserts;
    all.extend(la_asserts);
    let adiags = sigil_link::check_link_asserts(&resolved, &SymbolTable::new(), &all);
    assert!(
        adiags.iter().all(|d| d.level != sigil_span::Level::Error),
        "tranche20 mixed drift guards: {adiags:?}"
    );
...
#[test]
fn mixed_tranche20_rom_matches_assembled_reference() {
    let aeon = aeon_dir();
    let rom_path = aeon.join("s4.bin");
    let Ok(refrom) = std::fs::read(&rom_path) else {
        if strict_gate() {
    ...
    let rom = build_mixed_tranche20_rom(&aeon, false);
    assert_rom_matches_convsym(&rom, &refrom, ASSEMBLED_LEN, "tranche20 mixed");
}

#[test]
fn mixed_tranche20_debug_rom_matches_assembled_reference() {
    let aeon = aeon_dir();
    let rom_path = aeon.join("s4.debug.bin");
    let Ok(refrom) = std::fs::read(&rom_path) else {
        if strict_gate() {
