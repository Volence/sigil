...
pub const COMPRESSION_SELFTEST: Region = Region { plain_base: 0x62CE, debug_base: 0x7176, plain_len: 0x0, debug_len: 0xC8A };
...
/// `NullInterrupt` .. start + 0x2 (literal — the no-op IRQ handler rte, just before error_handler) — gate `SIGIL_EMP_NULL_INTERRUPT`. tests: m1d_rom, m1d_debug_rom
...
/// `ObjCodeBase` .. start + 0x2 (literal — the object-bank base + the offset-0 safety rts) — gate `SIGIL_EMP_OBJCODEBASE`. tests: m1d_rom, m1d_debug_rom
