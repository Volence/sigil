...
import os as _os
ANCHORS = json.loads(_os.environ.get("AB_ANCHORS", "[240, 420, 700]"))
ROM_TOP = 0x70000
...
async def main():
    # ONE ROM per process invocation (argv: OLD|NEW); the GUI is restarted between
    # ROMs and the single reload happens here, fresh — the unload-race mitigation.
    name = sys.argv[1]
    rom = ROMS[name]
    ...
    with open(f"{OUT}/manifest_pb4_{name}.json", "w") as f:
        json.dump(m, f, indent=1)
    # The fb hash is the GUI's render-thread snapshot — wall-clock racy under scroll
    # motion (which frame it holds at pause is host-timing dependent). Emulated state
    # (RAM + VRAM/CRAM/VSRAM/regs) is the determinism bar; pixels are a pure function
    # of that state.
    strip = lambda recs: [{k: v for k, v in r.items() if k != "fb"} for r in recs]
    assert strip(m[f"{name}_run1"]) == strip(m[f"{name}_run2"]), f"{name} runs differ — determinism broken"
    print(f"{name}: DETERMINISM OK (emulated state; fb snapshot excluded as racy)")
    print("DONE")
