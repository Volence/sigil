...
manifest_scroll_<name>.json. Run once for OLD, once for NEW, then diff the manifests.
...
async def one_run(bus, idx):
    d = f"{OUT}/scroll_{NAME}_run{idx}"
    os.makedirs(d, exist_ok=True)
    await call(bus, "breakpoint_clear", {"all": True})
    await call(bus, "reset", {"run": False})
    await call(bus, "run_frames", {"frames": 60})
    # NO B-press: stay in debug-fly and hold right = the max-H SCROLLING drive.
    # The camera moves every frame, so Section_UpdateColumns + the parallax
    # HScroll/VSRAM build run against a moving world (the deform machinery is
    # exercised, unlike the wall-pinned collision scene).
    await call(bus, "hold", {"buttons": ["right"], "down": True})
    await call(bus, "run_frames", {"frames": 122})
    recs, cur = [], 182
    for a in ANCHORS:
...
async def main():
    bus = BusClient(client_id="wcscr", client_name="wavec-scroll-ab", client_version="1", want_events=False)
    await bus.connect()
    await call(bus, "breakpoint_clear", {"all": True})
    ...
    await verify_cart_bus(bus, ROM, label=f"{NAME} step 0")
    ...
    with open(f"{OUT}/manifest_scroll_{NAME}.json", "w") as f:
