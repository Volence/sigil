...
## Stage-3 P2d RE-FREEZE — the keystone flip + the appendix demangler (2026-07-30)

The five keystone code twins flip native (`player_common`, `test_player`, `test_enemy`,
`act_descriptor` join the registry; `z80_init` places natively in the no-sound
config_b/demo shapes — kill rows 55/93) and a listing-side demangler rewrites the
sigil-canonical `.emp` locals for the deb2 appendix (OQ-B). This moves ONLY the
full-file / appendix layer; **the six assembled anchors do NOT move** — that is the
invariant that makes the re-freeze safe.

**PRIMARY PROVENANCE — the UNMOVED assembled anchors** (header-neutral CRC of
`[0, EndOfRom)`, recomputed directly from the re-frozen goldens; these NEVER move):

| target | anchor (unmoved) | EndOfRom |
|---|---|---|
| s4.bin (sonic4 plain) | `e5765873` | 0x5db60 |
| s4.debug.bin (sonic4 debug) | `dab4f06c` | 0x5f65a |
| demo.bin | `cfda98d3` | 0x11224 |
| demo.debug.bin | `20c5571d` | 0x11224 |
| config_a.bin | `3d9bac53` | 0x5f65a |
| config_b.bin | `fd3f7f8e` | 0x434d0 |

**The MOVED full-file goldens** (sigil-canonical; the appendix follows the flipped
symbol set — the "re-freeze the golden" signal, split-golden model). Regenerated by
`capture_goldens.sh --write` (config_a/config_b now via `sigil build --config-*`
directly — the stale SOUND_* env path through build.sh retired):

| target | OLD full crc/size | NEW full crc/size | appendix bytes old→new | deb2 syms old→new |
|---|---|---|---|---|
| s4.bin | `2198deb2`/395374 | `7f071417`/412306 | 11534→28466 | 676→1456 |
| s4.debug.bin | `1d895fcb`/402696 | `0b8efc7a`/422147 | 11950→31401 | 702→1531 |
| config_a.bin | `80e602df`/402742 | `1b4c49d2`/422483 | 11996→31737 | 705→1547 |
| config_b.bin | `9eb2e8a1`/286904 | `bfe2509e`/303660 | 11240→27996 | 650→1421 |
| demo.bin | `0646d4bf`/76851 | `705a5871`/90436 | 6671→20256 | 373→1000 |
| demo.debug.bin | `7e4a358a`/77244 | `37ded207`/92935 | 7064→22755 | 397→1060 |

(The OLD `config_a.bin`/`config_b.bin`/`demo*.bin` golden blobs were asl-derived full
files whose PREFIX was the anchor; the re-freeze replaces them with the sigil-native
full files — the prefix is still the anchor, so the anchor gates are unchanged, and the
goldens now match what `sigil build` ships.)

**The demangler policy (OQ-B).** `sigil_link::demangle_symbols`, applied to the
convsym-fed listing in `append_deb2_appendix` (ROM-byte-neutral): `$module$Parent$local`
and `__offsets$module$Parent$local` → `Parent.local` (KEPT — they now survive convsym's
`as_lst` name parser); `__align$…` and any `asm<N>` block-scope name → DROPPED. Sample:
`$engine.boot$EntryPoint$wait_dma`→`EntryPoint.wait_dma`,
`__offsets$…$Ani_Sonic$Walk`→`Ani_Sonic.Walk`, `$engine.boot$asm1$wait_z80`→dropped.
The appendix grows to a genuine debugger symbol set (676→1456 for sonic4 plain).

**The OFF-CANONICAL SIZE TABLES stay UNCHANGED** (only each `# golden_crc32=` provenance
tie is updated to the re-frozen golden). The chainer's frozen tables carry ROM/LMA
addresses; they are correct and STABLE under the flip because the assembled anchors are
unmoved (verified: `native_offcanonical_placement`/anchor gates green with the committed
tables). A full sigil-native RE-DERIVATION of the tables from sigil's own resolve is NOT
possible yet and is DEFERRED to P4 (the map-manifest placement authority, rows 34/95):
the sigil `.lst` lists a phased section's labels at their VMA, so a naive re-capture reads
`Z80_IdleProgram` at `0x0` instead of its ROM address `0x3d8` (and drops section-end
labels like `Ani_Sonic_End`) — a re-captured table breaks `config_b_frozen_placement_exact`.
P4 re-derives them from the resolved LMAs, closing the asl-derived-constants era.
