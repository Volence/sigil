...
fn link_with_truths(
    ...
    // Region geometry reads `pins` (regenerated by repin from the build's own
    // listing), so a cartridge re-layout cannot leave a stale base here. The
    // probes run the plain-shape lowering; `test_particle` is a DEBUG-only
    // region (`plain_len` is 0), so its extent comes from `debug_len` the way
    // the port gate's compile harness takes it.
    let solid_base = pins::TEST_SOLID.plain_base;
    let solid_size = pins::TEST_SOLID.plain_len;
    let particle_base = pins::TEST_PARTICLE.plain_base;
    let particle_size = pins::TEST_PARTICLE.debug_len;
    let map_toml = format!(
        "fill = 0x00\n\
         \n\
         [[region]]\n\
