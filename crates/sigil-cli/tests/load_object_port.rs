...
fn compile_real_file(
    ...
    let file = with_ambient(vec![types, sst, constants, frames], lo);
...
/// (plain) the `load_object` region == the `s4.bin` window at
/// `pins::LOAD_OBJECT`'s plain base/len — DEBUG=0.
#[test]
fn load_object_region_matches_reference() {
    reference_gate(&PLAIN, "s4.bin", 0);
}

/// (debug) the `load_object` region == the `s4.debug.bin` window at
/// `pins::LOAD_OBJECT`'s debug base/len — DEBUG=1
