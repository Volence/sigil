...
/// at its TRUE per-shape VMA (plain `$431E`, debug `$4A8A` —
...
fn as_tile_cache_label(debug: bool) -> Vec<Section> {
    let base = if debug { "$4A8A" } else { "$431E" };
    let asm = format!(
        "cpu 68000\n\
         phase {base}\n\
...
fn collision_lookup_region_matches_reference() {
    ...
    let expected_disp = (0x4C06i64 - (consumer.lma as i64 + 2)) as i16;
    assert_eq!(
        disp, expected_disp,
        "bare-name proof: `bsr.w Collision_GetType` must resolve to $4C06 (plain)"
    );
}
...
fn collision_lookup_debug_region_matches_reference() {
    ...
    let consumer = linked
        ...
        .expect("linked image must carry the outbound consumer at its harness-private LMA");
    let disp = i16::from_be_bytes([consumer.bytes[2], consumer.bytes[3]]);
    let expected_disp = (0x542Ai64 - (consumer.lma as i64 + 2)) as i16;
    assert_eq!(
        disp, expected_disp,
        "bare-name proof: `bsr.w Collision_GetType` must resolve to $542A (debug)"
    );
}
