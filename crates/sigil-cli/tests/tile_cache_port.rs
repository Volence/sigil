...
fn tile_cache_value_equs() -> Vec<Section> {
    let mut pairs: Vec<(&str, &str)> = vec![
        ...
        // (hoisted into the shared engine.constants twin, prepended above).
    ...
    // Act_*/Sec_* field equs feed the prepended engine.structs drift wall;
    // engine_constant_equs() feeds the prepended engine.constants twin's wall
    // (incl. the hoisted TILE_CACHE_*).
...
fn tile_cache_addr_labels(debug: bool) -> Vec<Section> {
    ...
    let mut table: Vec<(&str, u32)> = vec![
        ...
        ("Block_Stage_Gen", pick(pins::BLOCK_STAGE_GEN)),
        ("Pfx_Memo_Row", pick(pins::PFX_MEMO_ROW)),
        ("Pfx_Memo_L16", pick(pins::PFX_MEMO_L16)),
        ("Pfx_Memo_H16", pick(pins::PFX_MEMO_H16)),
        ("Pfx_Memo_Gen", pick(pins::PFX_MEMO_GEN)),
        ("Cs_Memo_Col", pick(pins::CS_MEMO_COL)),
        ("Cs_Memo_T16", pick(pins::CS_MEMO_T16)),
        ("Cs_Memo_B16", pick(pins::CS_MEMO_B16)),
        ("Cs_Memo_Gen", pick(pins::CS_MEMO_GEN)),
        // Streaming-arc F2: the guard ring + rekeyed memo masks.
        ("Pfx_Memo_Mask", pick(pins::PFX_MEMO_MASK)),
        ("Cs_Memo_Mask", pick(pins::CS_MEMO_MASK)),
        ("Cache_Spec_Gen_Ring", pick(pins::CACHE_SPEC_GEN_RING)),
        ("Cache_Spec_Window", pick(pins::CACHE_SPEC_WINDOW)),
        ("Cache_Spec_Blocked", pick(pins::CACHE_SPEC_BLOCKED)),
        ("Logic_Tick", pick(pins::LOGIC_TICK)),
        ("Cache_Spec_Skips", pick(pins::CACHE_SPEC_SKIPS)),
...
fn compile_real_file(
    ...
    // tile_cache.emp `use`s engine.structs.{Act, Sec, Act_grid_w_lo, Act_grid_h_lo}
    // AND engine.constants.{TILE_CACHE_*} — prepend both (the shared struct module's
    // Sec/Act layout + drift wall + grid-lo consts, and the engine.constants twin).
...
fn tile_cache_value_pairs() -> Vec<(&'static str, &'static str)> {
    vec![
        // TILE_CACHE_COLS/ROWS/STRIDE/NT_SIZE come from engine_constant_equs (cl_twin) in the union.
        ("TILE_CACHE_COLL_ROWS", "30"),
