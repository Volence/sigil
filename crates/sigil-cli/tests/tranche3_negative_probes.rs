...
/// `$4320`; probe (d) supplies a WRONG one.
...
fn link_collision(src: &str, twin: &str, tile_cache_vma: &str) -> sigil_link::LinkedImage {
    let mut sections = place_module(src, twin, "collision_lookup", "0x4C08", "0x24");
...
///
/// FALSIFIED (restore-real-value): placing at the real `0x4C08` yields
/// `lma == 0x4C08` (the port gate's compile path).
#[test]
fn collision_lookup_wrong_base_map_places_the_section_at_a_different_address() {
    ...
    assert_ne!(sec.lma, 0x4C08, "…and therefore differ from the true pin");
}
...
/// phased at a WRONG VMA (`$4324`, +4 from the genuine `$4320`): the
...
///
/// FALSIFIED (restore-real-value): with the genuine `$4320` the linked bytes
...
fn collision_lookup_wrong_tile_cache_vma_changes_the_bra_bytes() {
    ...
    let Some(twin) = twin_src() else { return };
    let Some(refrom) = read_reference("s4.bin") else { return };
    let linked = link_collision(&src, &twin, "$4324");
    let section = linked.section("collision_lookup").expect("collision_lookup section");
    assert_ne!(
        section.bytes,
        &refrom[0x4C08..0x4C2C],
