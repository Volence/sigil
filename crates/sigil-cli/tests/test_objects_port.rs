...
fn as_constant_equs() -> Vec<Section> {
    ...
    // The game-config values test_objects.emp's D11 drift guards check — each
    // lives in a DIFFERENT AS file (config/constants.asm, engine/constants.asm)
    // that survives the test_objects.asm twin. (ENEMY_PATROL_SPEED's AS home
    // test_enemy.asm was deleted at conv-d #47 — it is now owned in-module, no
    // extern guard, so no equ supply.)
