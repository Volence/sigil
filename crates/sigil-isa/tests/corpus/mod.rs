...
pub fn corpus() -> Vec<(&'static str, Instruction)> {
    vec![
        ...
        ("sub (ix+3)", inst(Sub, vec![Indexed { reg: IxR, disp: 3 }])),
        ("inc ix", inst(Inc, vec![Pair(Ix)])),
        ("dec ix", inst(Dec, vec![Pair(Ix)])),
        ("ld (1234h),ix", inst(Ld, vec![Mem(0x1234), Pair(Ix)])),
        // --- FD (iy) ---
        ("ld iy,1234h", inst(Ld, vec![Pair(Iy), Imm16(0x1234)])),
        ("inc iy", inst(Inc, vec![Pair(Iy)])),
        ("ld (1234h),iy", inst(Ld, vec![Mem(0x1234), Pair(Iy)])),
