...
// `pub(crate)` so the `authored_rail_holes` census walk in `corpus_contracts`
// tests a would-be hole against the SAME resolver the closure's own hole
// determination uses (below), never a hand-rolled second copy.
