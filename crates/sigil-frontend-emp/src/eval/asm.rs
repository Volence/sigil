...
impl Evaluator<'_> {
    ...
    fn try_bare_statement_call(&mut self, instr: &InstrLine, env: &mut Env) -> Option<CodeBuf> {
        ...
        if let Some(kind) = self.bareword_non_fn_kind(name) {
            self.error(
                instr.span,
                format!(
                    "`{name}` is a {kind}, not a comptime fn — only comptime fn calls are legal \
    ...
    /// known non-fn comptime construct, for the specific "X is a <kind>" error
    /// (D-PP.1). Returns `None` for a name that resolves to nothing (which keeps
    /// today's mnemonic error). A `const` is reported as `const`; a type name
    /// (struct/enum/newtype/bitfield) by its category.
